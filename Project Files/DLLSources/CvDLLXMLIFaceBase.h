...
public:
	...
	virtual bool LocateFirstSiblingNodeByTagName(FXml* xml, char* pszTagName) = 0;
	// locate the next sibling node in this sub-tree layer that has the xml tag name,
	// returns true if a node is found and false otherwise
	virtual bool LocateNextSiblingNodeByTagName(FXml* xml, char* pszTagName) = 0;
	...
	virtual bool SetToChildByTagName(FXml* xml, char const* szTagName) = 0;
	...
	virtual bool GetLastNodeText(FXml* xml, char* pszText) = 0;
	...
	virtual int NumOfElementsByTagName(FXml* xml, char* pszTagName) = 0;
	// returns the number of children with the tag name
	virtual int NumOfChildrenByTagName(FXml* xml, char const* pszTagName) = 0;
	...
	virtual bool GetLastLocatedNodeTagName(FXml* xml, char* pszTagName) = 0;
	virtual bool IsAllowXMLCaching() = 0;
	virtual void MapChildren(FXml*) = 0;

	// overload for const correctness
	inline bool LocateFirstSiblingNodeByTagName(FXml* xml, char const* pszTagName) { return LocateFirstSiblingNodeByTagName(xml, (char*)pszTagName); }
};
