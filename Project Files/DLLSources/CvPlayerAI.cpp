...
CvPlayerAI* CvPlayerAI::m_aPlayers = nullptr;
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (iI != getID())
			{
				...
				{
					if (!(CvTeamAI::getTeam(getTeam()).isHuman()) && (CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman() || !(CvTeamAI::getTeam(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()).isHuman())))
					{
						if (CvTeamAI::getTeam(getTeam()).isAtWar(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()))
						{
							if (!(CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman()) || (CvTeamAI::getTeam(getTeam()).getLeaderID() == getID()))
							{
								FAssertMsg(iI != getID(), "iI is not expected to be equal with getID()");
								FAssert(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() != getTeam());

								if (CvTeamAI::getTeam(getTeam()).AI_getAtWarCounter(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()) > 10)
								{
									if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_PEACE_TREATY) == 0)
									{
										...
										if (!bOffered)
										{
											...
											{
												setTradeItem(&item, TRADE_PEACE_TREATY, 0, nullptr);

												if (canTradeItem(((PlayerTypes)iI), item, true) && CvPlayerAI::getPlayer((PlayerTypes)iI).canTradeItem(getID(), item, true))
												{
													iOurValue = CvTeamAI::getTeam(getTeam()).AI_endWarVal(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam());
													iTheirValue = CvTeamAI::getTeam(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()).AI_endWarVal(getTeam());
													iReceiveGold = 0;
													iGiveGold = 0;

													pBestReceiveCity = nullptr;
													pBestGiveCity = nullptr;
													...
													if (iTheirValue > iOurValue)
													{
														...
														iGold = std::min((iTheirValue - iOurValue), CvPlayerAI::getPlayer((PlayerTypes)iI).AI_maxGoldTrade(getID()));

														if (iGold > 0)
														{
															setTradeItem(&item, TRADE_GOLD, iGold, nullptr);

															if (CvPlayerAI::getPlayer((PlayerTypes)iI).canTradeItem(getID(), item, true))
															{
																iReceiveGold = iGold;
																iOurValue += iGold;
															}
														...
														if (iTheirValue > iOurValue)
														{
															...
															for (pLoopCity = CvPlayerAI::getPlayer((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = CvPlayerAI::getPlayer((PlayerTypes)iI).nextCity(&iLoop))
															{
																setTradeItem(&item, TRADE_CITIES, pLoopCity->getID(), nullptr);
													...
													else if (iOurValue > iTheirValue)
													{
														...
														for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
														{
															...
															if (canTradeItem(((PlayerTypes)iI), item, true))
															{
																if (CvPlayerAI::getPlayer((PlayerTypes)iI).AI_cityTradeVal(pLoopCity) <= (iOurValue - iTheirValue))
																{
														...
														if (pBestGiveCity != nullptr)
														{
															iTheirValue += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_cityTradeVal(pBestGiveCity);
														}
													}

													if ((CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman()) ? (iOurValue >= iTheirValue) : ((iOurValue > ((iTheirValue * 3) / 5)) && (iTheirValue > ((iOurValue * 3) / 5))))
													{
														ourList.clear();
														theirList.clear();
														...
														if (iGiveGold != 0)
														{
															setTradeItem(&item, TRADE_GOLD, iGiveGold, nullptr);
															ourList.insertAtEnd(item);
														}

														if (iReceiveGold != 0)
														{
															setTradeItem(&item, TRADE_GOLD, iReceiveGold, nullptr);
															theirList.insertAtEnd(item);
														}

														if (pBestGiveCity != nullptr)
														{
															setTradeItem(&item, TRADE_CITIES, pBestGiveCity->getID(), nullptr);
															ourList.insertAtEnd(item);
														}

														if (pBestReceiveCity != nullptr)
														{
															setTradeItem(&item, TRADE_CITIES, pBestReceiveCity->getID(), nullptr);
															theirList.insertAtEnd(item);
														}

														if (CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman())
														{
															if (!(abContacted[CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()]))
															{
																...
																abContacted[CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()] = true;
															}
														}
														else
														{
...
void CvPlayerAI::AI_updateFoundValues(bool bStartingLoc)
{
	...
	int iLoop;
	for (CvArea* pLoopArea = GC.getMap().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMap().nextArea(&iLoop))
	{
...
void CvPlayerAI::AI_updateAreaTargets()
{
	...
	for (pLoopArea = GC.getMap().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMap().nextArea(&iLoop))
	{
		if (!(pLoopArea->isWater()))
		{
			...
			{
				pLoopArea->setTargetCity(getID(), nullptr);
			}
			else
			{
...
int CvPlayerAI::AI_movementPriority(CvSelectionGroup* pGroup)
{
	...
	if (pHeadUnit != nullptr)
	{
		...
		if (pHeadUnit->canFight())
		{
			...
			iCurrCombat = pHeadUnit->currCombatStr(nullptr, nullptr);
								...
								if (kProfession.isWorkPlot())
								{
									...
									else
									{
										...
										else
										{
											...
											if (pBestWorkedPlot == nullptr || pWorkedPlot->calculateBestNatureYield(eYieldProducedType, getTeam()) < pBestWorkedPlot->calculateBestNatureYield(eYieldProducedType, getTeam()))
											{
												bRemove = true;
											}
										}
	...
	if (!hasBusyUnit())
	{
		...
		if (isHuman())
		{
			...
			while (pCurrUnitNode != nullptr)
			{
				...
				if (pLoopSelectionGroup == nullptr || pLoopSelectionGroup->AI_update())
				{
        ...
        else
		{
			...
			while (!m_unitPriorityHeap.empty())
			{
				...
				/*
				if (pUnit != nullptr && shouldUnitMove(pUnit))
				{
...
void CvPlayerAI::AI_groupUpdate()
{
	...
	while (tempGroupCycle.getLength() > 0)
	{
		...
		{
			pLoopSelectionGroup = getSelectionGroup(pCurrUnitNode->m_data);
			FAssertMsg(pLoopSelectionGroup != nullptr, "selection group node with null selection group");
	...
	while (pCurrUnitNode != nullptr)
	{
		...
		if (pLoopSelectionGroup != nullptr)  // group might have been killed by a previous group update
		{
			...
			if (GC.getGameINLINE().getElapsedGameTurns() > 20)
			{
				if (getNumCities() > 4)
				{
					...
					if (pNearestCity == nullptr)
					{
						...
						{
							if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(pCity->getPreviousOwner()).getTeam()).countNumCitiesByArea(pCity->area()) > 3)
							{
								iRazeValue += 30;
							}
						}
					...
					if (CvPlayerAI::getPlayer(pCity->getPreviousOwner()).isHuman())
					{
...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc)
{
	...
	if (iMinRivalRange != -1)
	{
		for (int iDX = -(iMinRivalRange); iDX <= iMinRivalRange; iDX++)
		{
			for (int iDY = -(iMinRivalRange); iDY <= iMinRivalRange; iDY++)
			{
				...
				if (pLoopPlot != nullptr)
				{
					if (pLoopPlot->plotCheck(PUF_isOtherTeam, getID()) != nullptr)
					{
						return 0;
					}
				}
	...
	for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		...
		else if (pLoopPlot->isOwned() && !CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).isNative())
        {
	...
	for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		...
		if (pLoopPlot != nullptr)
		{
			...
			if (pLoopPlot->isOwned())
			{
				...
				else if (CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).getParent() != NO_PLAYER)
				{
	...
	for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
		...
		else
		{
			...
			if (!pLoopPlot->isCityRadius() || (pLoopPlot->isOwned() && CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).isNative()))
			{
	...
	if (bStartingLoc)
	{
				...
				if (pLoopPlot != nullptr)
				{
					if (pLoopPlot->isWater() || (pLoopPlot->area() == pArea))
					{
						if (plotDistance(iX, iY, pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE()) <= iRange)
						{
						    ...
						    int iTempValue = 0;
							iTempValue += pLoopPlot->calculatePotentialYield(YIELD_FOOD, nullptr, false) * 15;
							iTempValue += pLoopPlot->calculatePotentialYield(YIELD_FUR, nullptr, false) * 4; // R&R, ray, adjustment because of MYCP
							iTempValue += pLoopPlot->calculatePotentialYield(YIELD_WILD_FEATHERS, nullptr, false) * 4; // WTP, ray, adjustment because of MYCP
							iTempValue += pLoopPlot->calculatePotentialYield(YIELD_FRUITS, nullptr, false) * 4; // WTP, ray, adjustment because of MYCP
							iValue += iTempValue;
							if (iTempValue < 21)
							{
								iGreaterBadTile += 2;
		...
		for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
		{
            ...
            if (pLoopPlot != nullptr)
		    {
		        ...
		        {
		            iWaterCount ++;
		            if (pLoopPlot->calculatePotentialYield(YIELD_FOOD, nullptr, false) <= 1)
		            {
		                iWaterCount++;
					}
				}
		...
		if (!pPlot->isStartingPlot())
		{
			...
			for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
				{
					if (iJ != getID())
					{
						int iClosenessFactor = CvPlayerAI::getPlayer((PlayerTypes)iJ).startingPlotDistanceFactor(pPlot, getID(), iMinRange);
...
{
	return ((pArea->getNumCities()) == CvTeamAI::getTeam(getTeam()).countNumCitiesByArea(pArea));
}
...
CvCity* CvPlayerAI::AI_findTargetCity(CvArea* pArea)
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			...
			if ((isPotentialEnemy(getTeam(), CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam())) || (AI_isPotentialRaidTarget((PlayerTypes)iI)))
			{
				for (pLoopCity = CvPlayerAI::getPlayer((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = CvPlayerAI::getPlayer((PlayerTypes)iI).nextCity(&iLoop))
				{
...
int CvPlayerAI::AI_goldTarget()
{
	...
	if (GC.getGameINLINE().getElapsedGameTurns() >= 40)
	{
		...
		bool bAnyWar = CvTeamAI::getTeam(getTeam()).getAnyWarPlanCount() > 0;
		if (bAnyWar)
		{
			iGold *= 3;
			iGold /= 2;
...
DiploCommentTypes CvPlayerAI::AI_getGreeting(PlayerTypes ePlayer)
{
	...
	if (CvPlayerAI::getPlayer(ePlayer).getTeam() != getTeam())
	{
		...
		if ((eWorstEnemy != NO_TEAM) && (eWorstEnemy != CvPlayerAI::getPlayer(ePlayer).getTeam()) && CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).isHasMet(eWorstEnemy) && (GC.getASyncRand().get(4) == 0))
		{
			if (CvPlayerAI::getPlayer(ePlayer).AI_hasTradedWithTeam(eWorstEnemy) && !atWar(CvPlayerAI::getPlayer(ePlayer).getTeam(), eWorstEnemy))
			{
...
bool CvPlayerAI::AI_isWillingToTalk(PlayerTypes ePlayer)
{
	...
	if (atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
		...
		if  (CvTeamAI::getTeam(getTeam()).AI_isChosenWar(CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
			if (!isNative())
			{
				iRefuseDuration *= 2;
		...
		int iOurSuccess = 1 + CvTeamAI::getTeam(getTeam()).AI_getWarSuccess(CvPlayerAI::getPlayer(ePlayer).getTeam());
		int iTheirSuccess = 1 + CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).AI_getWarSuccess(getTeam());
		...
		if (CvTeamAI::getTeam(getTeam()).AI_getAtWarCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) < iRefuseDuration)
		{
			return false;
		}
	}
...
bool CvPlayerAI::AI_demandRebukedSneak(PlayerTypes ePlayer)
{
	...
	FAssert(!(CvTeamAI::getTeam(getTeam()).isHuman()));
	...
	{
		if (CvTeamAI::getTeam(getTeam()).getPower() > CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getDefensivePower())
		{
			return true;
		}
	}
...
bool CvPlayerAI::AI_demandRebukedWar(PlayerTypes ePlayer)
{
	...
	if (GC.getASyncRand().get(100, "AI Demand Rebuked ASYNC") < GC.getLeaderHeadInfo(getPersonalityType()).getDemandRebukedWarProb())
	{
		if (CvTeamAI::getTeam(getTeam()).getPower() > CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getDefensivePower())
		{
			if (CvTeamAI::getTeam(getTeam()).AI_isAllyLandTarget(CvPlayerAI::getPlayer(ePlayer).getTeam()))
			{
				return true;
			}
		}
...
int CvPlayerAI::AI_getAttitudeVal(PlayerTypes ePlayer, bool bForced)
{
	...
	iAttitude += GC.getHandicapInfo(CvPlayerAI::getPlayer(ePlayer).getHandicapType()).getAttitudeChange();

	iAttitude -= std::max(0, (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getNumMembers() - CvTeamAI::getTeam(getTeam()).getNumMembers()));

	if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).AI_getWarSuccess(getTeam()) > CvTeamAI::getTeam(getTeam()).AI_getWarSuccess(CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
...
int CvPlayerAI::AI_getCloseBordersAttitude(PlayerTypes ePlayer)
{
	if (m_em_iCloseBordersAttitudeCache.get(ePlayer) == MAX_INT)
	{
		...
		if (CvTeamAI::getTeam(getTeam()).AI_isLandTarget(CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
			iPercent += 40;
		}
...
int CvPlayerAI::AI_getAlarmAttitude(PlayerTypes ePlayer)
{
	...
	int iAlarm = CvPlayerAI::getPlayer(ePlayer).getNumCities() * GC.getAlarmInfo(eAlarm).getNumColonies();
	iAlarm *= std::max(0, 100 + CvPlayerAI::getPlayer(ePlayer).getNativeAngerModifier());
	iAlarm /= 100;
...
int CvPlayerAI::AI_getRebelAttitude(PlayerTypes ePlayer)
{
	...
	int iBells = CvPlayerAI::getPlayer(ePlayer).getBellsStored();
	...
	const int iRebelPercent = CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getRebelPercent();
...
int CvPlayerAI::AI_getWarAttitude(PlayerTypes ePlayer)
{
	...
	{
		int iAttitudeChange = (CvTeamAI::getTeam(getTeam()).AI_getAtWarCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) / GC.getLeaderHeadInfo(getPersonalityType()).getAtWarAttitudeDivisor());
...
int CvPlayerAI::AI_getPeaceAttitude(PlayerTypes ePlayer)
{
	...
	{
		int iAttitudeChange = (CvTeamAI::getTeam(getTeam()).AI_getAtPeaceCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) / GC.getLeaderHeadInfo(getPersonalityType()).getAtPeaceAttitudeDivisor());
...
int CvPlayerAI::AI_getOpenBordersAttitude(PlayerTypes ePlayer)
{
	if (!atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
		...
		{
			int iAttitudeChange = (CvTeamAI::getTeam(getTeam()).AI_getOpenBordersCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) / GC.getLeaderHeadInfo(getPersonalityType()).getOpenBordersAttitudeDivisor());
...
int CvPlayerAI::AI_getDefensivePactAttitude(PlayerTypes ePlayer)
{
	if (!atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
		...
		{
			int iAttitudeChange = (CvTeamAI::getTeam(getTeam()).AI_getDefensivePactCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) / GC.getLeaderHeadInfo(getPersonalityType()).getDefensivePactAttitudeDivisor());
...
int CvPlayerAI::AI_getRivalDefensivePactAttitude(PlayerTypes ePlayer)
{
	...
	if (!(CvTeamAI::getTeam(getTeam()).isDefensivePact(CvPlayerAI::getPlayer(ePlayer).getTeam())))
	{
		iAttitude -= ((4 * CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getDefensivePactCount(CvPlayerAI::getPlayer(ePlayer).getTeam())) / std::max(1, (GC.getGameINLINE().countCivTeamsAlive() - 2)));
	}
...
int CvPlayerAI::AI_getShareWarAttitude(PlayerTypes ePlayer)
{
	...
	if (!atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
		if (CvTeamAI::getTeam(getTeam()).AI_shareWar(CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
		...
		{
			iAttitudeChange = (CvTeamAI::getTeam(getTeam()).AI_getShareWarCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) / GC.getLeaderHeadInfo(getPersonalityType()).getShareWarAttitudeDivisor());
...
{
	// XXX human only?
	return range(((AI_getPeacetimeGrantValue(ePlayer) + std::max(0, (AI_getPeacetimeTradeValue(ePlayer) - CvPlayerAI::getPlayer(ePlayer).AI_getPeacetimeTradeValue(getID())))) / ((CvTeamAI::getTeam(getTeam()).AI_getHasMetCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) + 1) * 5)), 0, 4);
}

int CvPlayerAI::AI_getRivalTradeAttitude(PlayerTypes ePlayer)
{
	// XXX human only?
	return -(range(((CvTeamAI::getTeam(getTeam()).AI_getEnemyPeacetimeGrantValue(CvPlayerAI::getPlayer(ePlayer).getTeam()) + (CvTeamAI::getTeam(getTeam()).AI_getEnemyPeacetimeTradeValue(CvPlayerAI::getPlayer(ePlayer).getTeam()) / 3)) / ((CvTeamAI::getTeam(getTeam()).AI_getHasMetCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) + 1) * 10)), 0, 4));
}
...
int CvPlayerAI::AI_dealVal(PlayerTypes ePlayer, const CLinkList<TradeData>* pList, bool bIgnoreAnnual, int iChange)
{
	...
	if (atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
	{
		iValue += CvTeamAI::getTeam(getTeam()).AI_endWarVal(CvPlayerAI::getPlayer(ePlayer).getTeam());
	}
	...
	for (CLLNode<TradeData>* pNode = pList->head(); pNode; pNode = pList->next(pNode))
	{
		...
		{
		case TRADE_CITIES:
			iValue += AI_cityTradeVal(CvPlayerAI::getPlayer(ePlayer).getCity(pNode->m_data.m_iData1));
			break;
		case TRADE_GOLD:
		...
		case TRADE_YIELD:
			...
			break;
		case TRADE_MAPS:
			iValue += CvTeamAI::getTeam(getTeam()).AI_mapTradeVal(CvPlayerAI::getPlayer(ePlayer).getTeam());
			break;
		case TRADE_OPEN_BORDERS:
			iValue += CvTeamAI::getTeam(getTeam()).AI_openBordersTradeVal(CvPlayerAI::getPlayer(ePlayer).getTeam());
			break;
		case TRADE_DEFENSIVE_PACT:
			iValue += CvTeamAI::getTeam(getTeam()).AI_defensivePactTradeVal(CvPlayerAI::getPlayer(ePlayer).getTeam());
			break;
		case TRADE_PEACE:
			iValue += CvTeamAI::getTeam(getTeam()).AI_makePeaceTradeVal(((TeamTypes)(pNode->m_data.m_iData1)), CvPlayerAI::getPlayer(ePlayer).getTeam());
			break;
		case TRADE_WAR:
			iValue += CvTeamAI::getTeam(getTeam()).AI_declareWarTradeVal(((TeamTypes)(pNode->m_data.m_iData1)), CvPlayerAI::getPlayer(ePlayer).getTeam());
			break;
		case TRADE_EMBARGO:
...
bool CvPlayerAI::AI_considerOffer(PlayerTypes ePlayer, const CLinkList<TradeData>* pTheirList, const CLinkList<TradeData>* pOurList, int iChange)
{
	...
	int iOurValue = CvPlayerAI::getPlayer(ePlayer).AI_dealVal(getID(), pOurList, false, iChange);
	...
	if (iOurValue > 0 && 0 == pTheirList->getLength() && 0 == iTheirValue)
	{
		...
		{
			if (CvTeamAI::getTeam(getTeam()).getPower() > ((CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getPower() * 4) / 3))
			{
				return false;
			}
		}
		...
		iThreshold = (CvTeamAI::getTeam(getTeam()).AI_getHasMetCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) + 50);

		iThreshold *= 2;

		if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).AI_isLandTarget(getTeam()))
		{
			iThreshold *= 3;
		}

		iThreshold *= (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).getPower() + 100);
		iThreshold /= (CvTeamAI::getTeam(getTeam()).getPower() + 100);

		iThreshold -= CvPlayerAI::getPlayer(ePlayer).AI_getPeacetimeGrantValue(getID());
...
{
	FAssert(CvPlayerAI::getPlayer(ePlayer).getParent() == getID());
...
bool CvPlayerAI::AI_counterPropose(PlayerTypes ePlayer, const CLinkList<TradeData>* pTheirList, const CLinkList<TradeData>* pOurList, CLinkList<TradeData>* pTheirInventory, CLinkList<TradeData>* pOurInventory, CLinkList<TradeData>* pTheirCounter, CLinkList<TradeData>* pOurCounter, const IDInfo& kTransport)
{
	...
	bool acceptsBargaining = CvPlayerAI::getPlayer(getID()).isWillingToBargain();
	...
	if(acceptsBargaining)
	{
		if (bOurGoldDeal)
		{
			...
			if (pNode->m_data.m_eItemType == TRADE_GOLD)
			{
				...
				newPrice = newPrice * (100 + CvPlayerAI::getPlayer(getID()).getNativeTradeModifier()) / 100;

				// R&R, ray, small correction to stop at max gold of player
				int iGoldAvailable = CvPlayerAI::getPlayer(getID()).AI_maxGoldTrade(ePlayer);
		...
		CvPlayerAI::getPlayer(getID()).setWillingToBargain(false);
		return true;
	}
	...
	iAIDealWeight = CvPlayerAI::getPlayer(ePlayer).AI_dealVal(getID(), pOurList);
	...
	if (iAIDealWeight > iHumanDealWeight)
	{
		if (atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
			...
			for (pNode = pTheirInventory->head(); pNode && iAIDealWeight > iHumanDealWeight; pNode = pTheirInventory->next(pNode))
			{
				if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden)
				{
					...
					{
						FAssert(CvPlayerAI::getPlayer(ePlayer).canTradeItem(getID(), pNode->m_data));

						if (CvPlayerAI::getPlayer(ePlayer).getTradeDenial(getID(), pNode->m_data) == NO_DENIAL)
						{
							pCity = CvPlayerAI::getPlayer(ePlayer).getCity(pNode->m_data.m_iData1);
		...
		if (iGoldWeight > 0)
		{
			if (pGoldNode)
			{
				...
				if (CvPlayerAI::getPlayer(ePlayer).getMaxGoldTrade(getID(), kTransport) >= iGoldData)
				{
		...
		for (pNode = pTheirInventory->head(); pNode && iAIDealWeight > iHumanDealWeight; pNode = pTheirInventory->next(pNode))
		{
			if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden)
			{
				if (pNode->m_data.m_eItemType == TRADE_MAPS)
				{
					...
					if (CvPlayerAI::getPlayer(ePlayer).getTradeDenial(getID(), pNode->m_data) == NO_DENIAL)
					{
						iWeight = CvTeamAI::getTeam(getTeam()).AI_mapTradeVal(CvPlayerAI::getPlayer(ePlayer).getTeam());
		...
		if (iGoldWeight > 0)
		{
			if (pGoldNode)
			{
				...
				iGoldData = std::min(iGoldData, CvPlayerAI::getPlayer(ePlayer).getMaxGoldTrade(getID(), kTransport));
				...
				if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden)
				{
					if (pNode->m_data.m_eItemType == TRADE_CITIES)
					{
						...
						if (getTradeDenial(ePlayer, pNode->m_data) == NO_DENIAL)
						{
							...
							if (pCity != nullptr)
							{
								iWeight = CvPlayerAI::getPlayer(ePlayer).AI_cityTradeVal(pCity);

								if (iWeight > 0)
								{
									const int iValue = CvPlayerAI::getPlayer(ePlayer).AI_targetCityValue(pCity, false);
		...
		for (pNode = pOurInventory->head(); pNode && iHumanDealWeight > iAIDealWeight; pNode = pOurInventory->next(pNode))
		{
			if (!pNode->m_data.m_bOffering && !pNode->m_data.m_bHidden)
			{
				if (pNode->m_data.m_eItemType == TRADE_MAPS)
				{
					...
					if (getTradeDenial(ePlayer, pNode->m_data) == NO_DENIAL)
					{
						iWeight = CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).AI_mapTradeVal(getTeam());
...
int CvPlayerAI::AI_maxGoldTrade(PlayerTypes ePlayer) const
{
	...
	if (isHuman() || (CvPlayerAI::getPlayer(ePlayer).getTeam() == getTeam()))
	{
		iMaxGold = getGold();
	}
	else
	{
		...
		iMaxGold *= (CvTeamAI::getTeam(getTeam()).AI_getHasMetCounter(CvPlayerAI::getPlayer(ePlayer).getTeam()) + 10);

		iMaxGold *= GC.getLeaderHeadInfo(getPersonalityType()).getMaxGoldTradePercent();
		iMaxGold /= 100;

		iMaxGold -= AI_getGoldTradedTo(ePlayer);
		iMaxGold += CvPlayerAI::getPlayer(ePlayer).AI_getGoldTradedTo(getID());
...
DenialTypes CvPlayerAI::AI_cityTrade(CvCity* pCity, PlayerTypes ePlayer) const
{
	...
	if (!(CvPlayerAI::getPlayer(ePlayer).isHuman()))
	{
		if (CvPlayerAI::getPlayer(ePlayer).getTeam() != getTeam())
		{
			if ((pCity->plot()->calculateCulturePercent(ePlayer) == 0) && !(pCity->isEverOwned(ePlayer)) && (CvPlayerAI::getPlayer(ePlayer).getNumCities() > 3))
			{
				...
				if (pNearestCity == nullptr || plotDistance(pCity->getX_INLINE(), pCity->getY_INLINE(), pNearestCity->getX_INLINE(), pNearestCity->getY_INLINE()) > 9)
				{
	...
	if (isNative() && !CvPlayerAI::getPlayer(ePlayer).isNative())
	{
		return NO_DENIAL;
	}
...
int CvPlayerAI::AI_stopTradingTradeVal(TeamTypes eTradeTeam, PlayerTypes ePlayer)
{
	...
	FAssertMsg(ePlayer != getID(), "shouldn't call this function on ourselves");
	FAssertMsg(CvPlayerAI::getPlayer(ePlayer).getTeam() != getTeam(), "shouldn't call this function on ourselves");
	FAssertMsg(eTradeTeam != getTeam(), "shouldn't call this function on ourselves");
	FAssertMsg(CvTeamAI::getTeam(eTradeTeam).isAlive(), "CvTeamAI::getTeam(eWarTeam).isAlive is expected to be true");
	FAssertMsg(!atWar(eTradeTeam, CvPlayerAI::getPlayer(ePlayer).getTeam()), "eTeam should be at peace with eWarTeam");

	iValue = (50 + (GC.getGameINLINE().getGameTurn() / 2));
	iValue += (CvTeamAI::getTeam(eTradeTeam).getNumCities() * 5);

	iModifier = 0;

	switch (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).AI_getAttitude(eTradeTeam))
	{
	...
	if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).isOpenBorders(eTradeTeam))
	{
		iValue *= 2;
	}

	if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).isDefensivePact(eTradeTeam))
	{
		iValue *= 3;
	}
	...
	for (pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != nullptr; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
	{
		...
		{
			if (CvPlayerAI::getPlayer(pLoopDeal->getFirstPlayer()).getTeam() == CvPlayerAI::getPlayer(ePlayer).getTeam())
			{
				if (pLoopDeal->getLengthSecondTrades() > 0)
				{
					iValue += (CvPlayerAI::getPlayer(pLoopDeal->getFirstPlayer()).AI_dealVal(pLoopDeal->getSecondPlayer(), pLoopDeal->getSecondTrades()) * ((pLoopDeal->getLengthFirstTrades() == 0) ? 2 : 1));
				}
			}

			if (CvPlayerAI::getPlayer(pLoopDeal->getSecondPlayer()).getTeam() == CvPlayerAI::getPlayer(ePlayer).getTeam())
			{
				if (pLoopDeal->getLengthFirstTrades() > 0)
				{
					iValue += (CvPlayerAI::getPlayer(pLoopDeal->getSecondPlayer()).AI_dealVal(pLoopDeal->getFirstPlayer(), pLoopDeal->getFirstTrades()) * ((pLoopDeal->getLengthSecondTrades() == 0) ? 2 : 1));
				}
			}
		}
	}
...
DenialTypes CvPlayerAI::AI_stopTradingTrade(TeamTypes eTradeTeam, PlayerTypes ePlayer) const
{
	...
	FAssertMsg(CvPlayerAI::getPlayer(ePlayer).getTeam() != getTeam(), "shouldn't call this function on ourselves");
	FAssertMsg(eTradeTeam != getTeam(), "shouldn't call this function on ourselves");
	FAssertMsg(CvTeamAI::getTeam(eTradeTeam).isAlive(), "CvTeamAI::getTeam(eTradeTeam).isAlive is expected to be true");
	...
	eAttitude = CvTeamAI::getTeam(getTeam()).AI_getAttitude(CvPlayerAI::getPlayer(ePlayer).getTeam());
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			...
			{
				if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getStopTradingRefuseAttitudeThreshold())
				{
	...
	eAttitudeThem = CvTeamAI::getTeam(getTeam()).AI_getAttitude(eTradeTeam);
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getTeam())
			{
				if (eAttitudeThem > GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getStopTradingThemRefuseAttitudeThreshold())
				{
...
{
	int iValue = 0;
	CvPlayerAI& kTradePlayer = CvPlayerAI::getPlayer(ePlayer);
...
//or if pCity is nullptr it assumes that it will either found somewhere
									...
									if (kProfessionInfo.isWorkPlot())
									{
										...
										for (int iX = -iRadius; iX <= iRadius; iX++)
										{
											for (int iY = -iRadius; iY <= iRadius; iY++)
											{
												...
												if (pLoopPlot != nullptr && pLoopPlot->getOwnerINLINE() == getID() && !pLoopPlot->isCity())
												{
													int iAmount = pLoopPlot->calculatePotentialYield(eYield, nullptr, false);
													...
													if (pLoopPlot->isBeingWorked())
													{
														...
														FAssert(pWorkingUnit != nullptr);

														if (pWorkingUnit != nullptr)
														{
...
int CvPlayerAI::AI_neededExplorers(CvArea* pArea)
{
	...
	if (0 == iNeeded)
	{
		if ((GC.getGameINLINE().countCivTeamsAlive() - 1) > CvTeamAI::getTeam(getTeam()).getHasMetCivCount())
		{
			if (pArea->isWater())
			{
			...
			{
				if (getPrimaryCity() != nullptr && pArea->getID() == getPrimaryCity()->getArea())
				{
					for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
					{
						CvPlayerAI& kPlayer = CvPlayerAI::getPlayer((PlayerTypes)iPlayer);
...
//Returns an estimate of needed pioneers. If pArea is nullptr then all areas will be considered (i.e. the total need of all our cities)
int CvPlayerAI::AI_neededWorkers(CvArea* pArea) const
{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		if (pArea == nullptr || pLoopCity->getArea() == pArea->getID())
		{
...
int CvPlayerAI::AI_plotTargetMissionAIs(CvPlot* pPlot, MissionAITypes* aeMissionAI, int iMissionAICount, int& iClosestTargetRange, CvSelectionGroup* pSkipSelectionGroup, int iRange)
{
	...
	CvSelectionGroup* pTransportSelectionGroup = nullptr;
	if (pSkipSelectionGroup != nullptr)
	{
		CvUnit* pHeadUnit = pSkipSelectionGroup->getHeadUnit();
		if (pHeadUnit->getTransportUnit() != nullptr)
		{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pSkipSelectionGroup == nullptr || (pLoopSelectionGroup != pSkipSelectionGroup && pLoopSelectionGroup != pTransportSelectionGroup))
		{
...
int CvPlayerAI::AI_cargoSpaceToEurope(CvSelectionGroup* pSkipSelectionGroup)
{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pSkipSelectionGroup == nullptr || pLoopSelectionGroup != pSkipSelectionGroup)
		{
			CvUnit* pHeadUnit = pLoopSelectionGroup->getHeadUnit();

			if (pHeadUnit != nullptr && pHeadUnit->AI_getUnitAIType() == UNITAI_TRANSPORT_SEA)
			{
...
int CvPlayerAI::AI_cityTargetUnitsByPath(CvCity* pCity, CvSelectionGroup* pSkipSelectionGroup, int iMaxPathTurns) const
{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pLoopSelectionGroup != pSkipSelectionGroup && pLoopSelectionGroup->plot() != nullptr && pLoopSelectionGroup->getNumUnits() > 0)
		{
			CvPlot* pMissionPlot = pLoopSelectionGroup->AI_getMissionAIPlot();
	...
	for(pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pLoopSelectionGroup != pSkipSelectionGroup)
		{
			if (pLoopSelectionGroup->AI_getMissionAIUnit() == pUnit)
			{
				...
				if (iMaxPathTurns >= 0 && pUnit->plot() != nullptr && pLoopSelectionGroup->plot() != nullptr)
				{
				...
				if ((iMaxPathTurns == -1) || (iPathTurns <= iMaxPathTurns))
				{
					for (int iMissionAIIndex = 0; iMissionAIIndex < iMissionAICount; iMissionAIIndex++)
					{
						if (eGroupMissionAI == aeMissionAI[iMissionAIIndex] || NO_MISSIONAI == aeMissionAI[iMissionAIIndex])
						{
							...
							if (pLoopSelectionGroup->getNumUnits() > 0 && pLoopSelectionGroup->getHeadUnit() != nullptr)
							{
								iCount += pLoopSelectionGroup->countNumUnitAIType(eUnitAI); //iCount += pLoopSelectionGroup->getHeadUnit() != nullptr ? pLoopSelectionGroup->countNumUnitAIType(eUnitAI) : 0;
							}
							// TAC - AI Attack City - koma13 - END
						}
					}
...
int CvPlayerAI::AI_enemyTargetMissionAIs(MissionAITypes* aeMissionAI, int iMissionAICount, CvSelectionGroup* pSkipSelectionGroup)
{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pSkipSelectionGroup == nullptr || (pLoopSelectionGroup != pSkipSelectionGroup && pLoopSelectionGroup != pTransportSelectionGroup))
		{
			...
			if (pMissionPlot != nullptr && pMissionPlot->isOwned())
			{
				MissionAITypes eGroupMissionAI = pLoopSelectionGroup->AI_getMissionAIType();
				for (int iMissionAIIndex = 0; iMissionAIIndex < iMissionAICount; iMissionAIIndex++)
				{
						if (eGroupMissionAI == aeMissionAI[iMissionAIIndex] || NO_MISSIONAI == aeMissionAI[iMissionAIIndex])
					{
						if (CvTeamAI::getTeam(getTeam()).AI_isChosenWar(pMissionPlot->getTeam()))
						{
							iCount += pLoopSelectionGroup->getNumUnits();
...
int CvPlayerAI::AI_wakePlotTargetMissionAIs(CvPlot* pPlot, MissionAITypes eMissionAI, CvSelectionGroup* pSkipSelectionGroup)
{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pSkipSelectionGroup == nullptr || (pLoopSelectionGroup != pSkipSelectionGroup && pLoopSelectionGroup != pTransportSelectionGroup))
		{
			...
			{
				CvPlot* pMissionPlot = pLoopSelectionGroup->AI_getMissionAIPlot();
				if (pMissionPlot != nullptr && pMissionPlot == pPlot)
				{
					iCount += pLoopSelectionGroup->getNumUnits();
...
// TAC - AI Economy - koma13 - END
{
	...
	// TAC - AI Economy - koma13 - START
	if (pArea != nullptr && pArea->getCitiesPerPlayer(getID()) <= 1)
	{
		pArea = nullptr;
	}
	// TAC - AI Economy - koma13 - END
	...
	for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		if (pArea == nullptr || pArea == pLoopCity->area())	// TAC - AI Economy - koma13
		{
			if (pLoopCity->plot()->getNearestEurope() != NO_EUROPE)
			{
				...
				if (pNoAccessCity == nullptr || kPlayer.AI_isTradeRouteConnection(pNoAccessCity, pLoopCity))
				{
				// TAC - AI Economy - koma13 - END
...
void CvPlayerAI::AI_changePeacetimeTradeValue(PlayerTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		if (iChange > 0)
		{
			if (CvPlayerAI::getPlayer(eIndex).getTeam() != getTeam())
			{
				for (iI = 0; iI < MAX_TEAMS; iI++)
				{
					if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
					{
						if (CvTeamAI::getTeam((TeamTypes)iI).AI_getWorstEnemy() == getTeam())
						{
							CvTeamAI::getTeam((TeamTypes)iI).AI_changeEnemyPeacetimeTradeValue(CvPlayerAI::getPlayer(eIndex).getTeam(), iChange);
						}
					}
				}
			}
			...
			if (CvPlayerAI::getPlayer(eIndex).getTeam() != getTeam())
			{
				for (iI = 0; iI < MAX_TEAMS; iI++)
				{
					if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
					{
						if (CvTeamAI::getTeam((TeamTypes)iI).AI_getWorstEnemy() == getTeam())
						{
							CvTeamAI::getTeam((TeamTypes)iI).AI_changeEnemyPeacetimeGrantValue(CvPlayerAI::getPlayer(eIndex).getTeam(), iChange);
						}
					}
				}
			}
...
void CvPlayerAI::AI_doTradeRoutes()
{
	...
	std::vector<CvCity*> yield_dests(NUM_YIELD_TYPES, nullptr);
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		...
		if (GC.getYieldInfo(eLoopYield).isCargo() && (eLoopYield != YIELD_FOOD))
		{
			...
			{
				yield_dests[eLoopYield] = nullptr;
			}
			...
			else
			{
				...
				for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
				{
					for (int iProfession = 0; iProfession < GC.getNumProfessionInfos(); ++iProfession)
					{
						...
						{
							int iValue = pLoopCity->getProfessionOutput((ProfessionTypes)iProfession, nullptr);
							if (iValue > 0)
							{
								iValue *= 100;
				...
				{
					// R&R, ray, catch nullptr-POINTER
					if (pBestYieldCity != nullptr)
					{
		...
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
		{
			...
			if (GC.getYieldInfo(eLoopYield).isCargo())
			{
				...
				// TAC - AI Economy - koma13 - END
				{
					...
					else if (yield_dests[eLoopYield] != nullptr)
					{
						// TAC - AI Economy - koma13 - START
						for (int iProfession = 0; iProfession < GC.getNumProfessionInfos(); ++iProfession)
						{
		...
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			...
			if (kPlayer.isAlive() && ePlayer != getID())
			{
				...
				if (kPlayer.isHuman() == (iPass == 1))
				{
					...
					{
						if (kPlayer.getTeam() != getTeam() && !(CvTeamAI::getTeam(getTeam()).isHuman()) && (kPlayer.isHuman() || !(CvTeamAI::getTeam(kPlayer.getTeam()).isHuman())))
						{
							FAssertMsg(iI != getID(), "iI is not expected to be equal with getID()");
				...
				if (bCancelDeal)
				{
					if (canContact(ePlayer) && AI_isWillingToTalk(ePlayer))
					{
						if (kPlayer.isHuman())
						{
							...
							for (CLLNode<TradeData>* pNode = pLoopDeal->headFirstTradesNode(); (pNode != nullptr); pNode = pLoopDeal->nextFirstTradesNode(pNode))
							{
							...
							for (CLLNode<TradeData>* pNode = pLoopDeal->headSecondTradesNode(); (pNode != nullptr); pNode = pLoopDeal->nextSecondTradesNode(pNode))
							{
			...
			if (((pLoopCity->getGameTurnAcquired() + 4) % 20) == (GC.getGameINLINE().getGameTurn() % 20))
			{
				...
				if (iCount >= (iPossibleCount / 2))
				{
					...
					if (canTradeItem((ePlayer), item, true))
					{
						...
						{
							GC.getGameINLINE().implementDeal(getID(), (ePlayer), &ourList, nullptr);
						}
						bOffered = true;
					}
				}
...
bool CvPlayerAI::AI_doDiploOfferAlliance(PlayerTypes ePlayer)
{
	...
	if (CvTeamAI::getTeam(getTeam()).getLeaderID() != getID())
	{
		return false;
	}
	...
	bool bOffered = false;
	TradeData item;
	setTradeItem(&item, TRADE_PERMANENT_ALLIANCE, 0, nullptr);
...
bool CvPlayerAI::AI_doDiploAskJoinWar(PlayerTypes ePlayer)
{
	...
	for (int iJ = 0; iJ < MAX_TEAMS; ++iJ)
	{
		...
		CvTeam& kLoopTeam = CvTeamAI::getTeam(eLoopTeam);
		if (kLoopTeam.isAlive())
		{
			if (atWar(eLoopTeam, getTeam()) && !atWar(eLoopTeam, kPlayer.getTeam()))
			{
				if (CvTeamAI::getTeam(kPlayer.getTeam()).isHasMet(eLoopTeam))
				{
					if (CvTeamAI::getTeam(kPlayer.getTeam()).canDeclareWar(eLoopTeam))
					{
	...
	pDiplo->addDiploCommentVariable(CvPlayerAI::getPlayer(CvTeamAI::getTeam(eBestTeam).getLeaderID()).getCivilizationAdjectiveKey());
	pDiplo->setAIContact(true);
...
bool CvPlayerAI::AI_doDiploAskStopTrading(PlayerTypes ePlayer)
{
	...
	TeamTypes eBestTeam = CvTeamAI::getTeam(getTeam()).AI_getWorstEnemy();
	if (eBestTeam == NO_TEAM)
	{
		return false;
	}

	if (!CvTeamAI::getTeam(kPlayer.getTeam()).isHasMet(eBestTeam))
	{
		return false;
	}

	if (CvTeamAI::getTeam(getTeam()).isParentOf(eBestTeam) && !::atWar(getTeam(), eBestTeam))
	{
		return false;
	}
...
bool CvPlayerAI::AI_doDiploGiveHelp(PlayerTypes ePlayer)
	...
	if (CvTeamAI::getTeam(kPlayer.getTeam()).getAssets() > CvTeamAI::getTeam(getTeam()).getAssets() / 2)
	{
		return false;
	}
...
bool CvPlayerAI::AI_doDiploDemandTribute(PlayerTypes ePlayer)
{
	...
	if (!CvTeamAI::getTeam(getTeam()).canDeclareWar(kPlayer.getTeam()))
	{
		return false;
	}

	if (CvTeamAI::getTeam(getTeam()).AI_isSneakAttackPreparing(kPlayer.getTeam()))
	{
		return false;
	}

	if (CvTeamAI::getTeam(kPlayer.getTeam()).getDefensivePower() >= CvTeamAI::getTeam(getTeam()).getPower())
	{
		return false;
	}
	...
	else if (CvTeamAI::getTeam(getTeam()).AI_mapTradeVal(kPlayer.getTeam()) > 100)
	{
		setTradeItem(&item, TRADE_MAPS, 0, nullptr);
	}
...
int CvPlayerAI::NBMOD_GetGoldAsk(PlayerTypes ePlayer) const
{
	...
	int iMaxGoldPlayerHas = CvPlayerAI::getPlayer(ePlayer).getGold();

	// first get the population and Attitude
	int iPopulation = CvPlayerAI::getPlayer(ePlayer).getTotalPopulation();
...
bool CvPlayerAI::AI_doDiploTradeMap(PlayerTypes ePlayer)
{
	...
	if (kPlayer.isHuman() && CvTeamAI::getTeam(getTeam()).AI_mapTradeVal(kPlayer.getTeam()) < CvTeamAI::getTeam(kPlayer.getTeam()).AI_mapTradeVal(getTeam()))
	{
		return false;
	}
...
bool CvPlayerAI::AI_doDiploDeclareWar(PlayerTypes ePlayer)
{
	...
	for (int iJ = 0; iJ < MAX_TEAMS; iJ++)
	{
		if (CvTeamAI::getTeam((TeamTypes)iJ).isAlive())
		{
			if (atWar(((TeamTypes)iJ), getTeam()))
			{
				int iAtWarCounter = CvTeamAI::getTeam(getTeam()).AI_getAtWarCounter((TeamTypes)iJ);
				if (CvTeamAI::getTeam(getTeam()).AI_getWarPlan((TeamTypes)iJ) == WARPLAN_DOGPILE)
				{
					iAtWarCounter *= 2;
					iAtWarCounter += 5;
				}
	...
	for (int iJ = 0; iJ < MAX_TEAMS; iJ++)
	{
		if (CvTeamAI::getTeam((TeamTypes)iJ).isAlive())
		{
			...
			{
				if (CvTeamAI::getTeam((TeamTypes)iJ).getAtWarCount() < std::max(2, (GC.getGameINLINE().countCivTeamsAlive() / 2)))
				{
					TradeData item;
					setTradeItem(&item, TRADE_WAR, iJ, nullptr);

					if (kPlayer.canTradeItem(getID(), item, true))
					{
						int iValue = (1 + GC.getGameINLINE().getSorenRandNum(1000, "AI Declare War Trading"));

						iValue *= (101 + CvTeamAI::getTeam((TeamTypes)iJ).AI_getAttitudeWeight(getTeam()));
						iValue /= 100;
	...
	iBestValue = 0;
	int iOurValue = CvTeamAI::getTeam(getTeam()).AI_declareWarTradeVal(eBestTeam, kPlayer.getTeam());
	int iTheirValue = 0;
	int iReceiveGold = 0;
	int iGiveGold = 0;
	...
	TradeData item;
	setTradeItem(&item, TRADE_WAR, eBestTeam, nullptr);
	theirList.insertAtEnd(item);
...
void CvPlayerAI::AI_doEurope()
{
				...
				// TAC - AI purchases military units - koma13 - END
				{
					...
					else
					{
						if (pLoopUnit->getProfession() == eDefaultProfession)
						{
							...
							{
								int iValue = AI_professionSuitability(pLoopUnit, eBuyProfession, nullptr);
	...
	for (int i = 0; i < getNumEuropeUnits(); i++)
	{
		...
		if (iNeeded > 0 || AI_isStrategy(STRATEGY_REVOLUTION_PREPARING))
		{
			...
			{
				eBestProfession = CvPlayerAI::getPlayer(pUnit->getOwnerINLINE()).AI_idealProfessionForUnitAIType(UNITAI_DEFENSIVE);
			}
			else
			{
				eBestProfession = CvPlayerAI::getPlayer(pUnit->getOwnerINLINE()).AI_idealProfessionForUnitAIType(UNITAI_COUNTER);
			}
	...
	*/
	...
	for (int i = 0; i < getNumEuropeUnits(); i++)
	{
		...
		if (kUnit.getTeacherWeight() <= 0 || AI_isStrategy(STRATEGY_REVOLUTION_PREPARING))
		{
			...
			if (eBuyUnit != NO_UNIT)
			{
				ProfessionTypes eProfession = CvPlayerAI::getPlayer(pUnit->getOwnerINLINE()).AI_idealProfessionForUnitAIType(eBuyUnitAI);
			...
			{
				if (AI_neededWorkers(nullptr) > 0)
				{
					ProfessionTypes eProfession = CvPlayerAI::getPlayer(pUnit->getOwnerINLINE()).AI_idealProfessionForUnitAIType(UNITAI_WORKER);
					if (eProfession != NO_PROFESSION && pUnit->canHaveProfession(eProfession, false))
					{
						changeProfessionEurope(pUnit->getID(), eProfession);
					}
				...
				else
				{
					if (GC.getGameINLINE().getSorenRandNum(100, "AI buy counter unit") < 25)
					{
						...
						{
							ProfessionTypes eProfession = CvPlayerAI::getPlayer(pUnit->getOwnerINLINE()).AI_idealProfessionForUnitAIType(UNITAI_COUNTER);
							if (eProfession != NO_PROFESSION && pUnit->canHaveProfession(eProfession, false))
							{
								changeProfessionEurope(pUnit->getID(), eProfession);
							}
...
void CvPlayerAI::AI_nativeYieldGift(CvUnit* pUnit)
{
	...
	if (pOtherCity != nullptr)
	{
		...
		if(iYieldAmount > 0)
		{
			...
			//popup dialog
			CvPlayer& kOtherPlayer = CvPlayerAI::getPlayer(pOtherCity->getOwnerINLINE());
			...
			if(!kOtherPlayer.isHuman())
			{
				if (kOtherPlayer.getGold() >= priceToPay)
				{
					...
					if (pHomeCity->getYieldStored(eBestYield) >= iYieldAmount)
					{
						...
						pHomeCity->changeYieldStored(eBestYield, -iYieldAmount);
						CvPlayerAI::getPlayer(pHomeCity->getOwnerINLINE()).changeGold(priceToPay);
					}
				}
			}
...
bool CvPlayerAI::AI_isYieldNeeded(YieldTypes eYield, int iCapacityPercent, CvCity* pCity) const
{
	...
	if (eYield == YIELD_HORSES)
	{
		...
		for (xLoopCity = firstCity(&xLoop); xLoopCity != nullptr; xLoopCity = nextCity(&xLoop))
		{
			if (pCity == nullptr || pCity == xLoopCity)
			{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		if (pCity == nullptr || pCity == pLoopCity)
		{
...
void CvPlayerAI::AI_manageEconomy()
{
	...
	bool bAtWar = (CvTeamAI::getTeam(getTeam()).getAnyWarPlanCount() > 0);
	int iLoop;
	for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
	...
	for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		for (int i = 0; i < GC.getNumProfessionInfos(); ++i)
		{
			...
			if (kProfession.isCitizen())
			{
				...
				if (eYield != NO_YIELD)
				{
					int iOutput = pLoopCity->getProfessionOutput((ProfessionTypes)i, nullptr);
...
CvPlot* CvPlayerAI::AI_getImperialShipSpawnPlot()
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; ++iI)
	{
		...
		{
			if (kPlayer.getStartingPlot() != nullptr)
			{
	...
	for (int iI = 0; iI < GC.getMap().numPlotsINLINE(); iI++)
	{
		...
		if (eEurope != NO_EUROPE)
		{
			if (bNoneAllowable || zoneAllowable[eEurope])
			{
				...
				{
					iValue /= 1 + AI_plotTargetMissionAIs(pLoopPlot, MISSIONAI_ASSAULT, nullptr, 1);
...
int CvPlayerAI::AI_highestProfessionOutput(ProfessionTypes eProfession, const CvCity* pIgnoreCity)
{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		...
		{
			iBestYield = std::max(iBestYield, pLoopCity->getProfessionOutput(eProfession, nullptr));

			//Also consider buildings under construction.
			CLLNode<OrderData>* pOrderNode = pLoopCity->headOrderQueueNode();

			while (pOrderNode != nullptr)
			{
...
int CvPlayerAI::AI_professionBasicValue(ProfessionTypes eProfession, UnitTypes eUnit, CvCity* pCity)
{
	...
	if (kProfession.isCitizen())
	{
		if (!kProfession.isWorkPlot())
		{
			int iNewOutput = pCity->AI_professionBasicOutput(eProfession, eUnit, nullptr);
			...
			for (int i = 0; i < pCity->getPopulation(); ++i)
			{
				...
				{
					int iOldOutput = pCity->AI_professionBasicOutput(eProfession, pLoopUnit->getUnitType(), nullptr);
		...
		else
		{
			for (int i = 0; i < NUM_CITY_PLOTS; ++i)
			{
				...
				if (pLoopPlot != nullptr && pLoopPlot->getWorkingCity() == pCity)
				{
	...
	for (int iI = 0; iI < GC.getNumProfessionInfos(); ++iI)
	{
		...
		if (!(kProfession.isCitizen() || kProfession.isWorkPlot()))
		{
			...
			if (GC.getCivilizationInfo(getCivilizationType()).isValidProfession(eLoopProfession))
			{
				...
				if (eUnitAI == UNITAI_SETTLER && kProfession.canFound() && pCity != nullptr)
				{
				...
				//WTP, ray, Settler Professsion - END
				{
					...
					if (pUnit == nullptr || pUnit->canHaveProfession(eLoopProfession, true))
					{
...
int CvPlayerAI::AI_unitAIValueMultipler(UnitAITypes eUnitAI)
{
	...
	switch (eUnitAI)
	{
		...
		case UNITAI_WORKER:
			if (!AI_isStrategy(STRATEGY_REVOLUTION_DECLARING))
			{
				int iNeeded = AI_neededWorkers(nullptr);
		...
		case UNITAI_DEFENSIVE:
			{
				...
				*/
				...
				int iUndefended = 0;
				int iNeeded = AI_totalDefendersNeeded(&iUndefended, nullptr, true);
...
{
	FAssert(pSourceCity != nullptr || pDestinationCity != nullptr);
...
int CvPlayerAI::AI_professionSuitability(const CvUnit* pUnit, ProfessionTypes eProfession, const CvPlot* pPlot, UnitAITypes eUnitAI) const
{
	...
	if (eUnitAI != NO_UNITAI && pUnit != nullptr)
	{
				...
				while (pUnitNode != nullptr)
				{
					...
					if (pLoopUnit->isEnemy(getTeam(), pLoopPlot))
					{
						if (!pLoopUnit->isDead())
						{
							if (eDomainType == NO_DOMAIN || pLoopUnit->getDomainType() == eDomainType)
							{
								strSum += pLoopUnit->currEffectiveStr(pLoopPlot, bAttack ? pLoopUnit : nullptr);
							}
						}
					}
				}
...
int CvPlayerAI::AI_eventValue(EventTypes eEvent, const EventTriggeredData& kTriggeredData)
{
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		if (kEvent.isDeclareWar())
		{
			...
			case ATTITUDE_ANNOYED:
			case ATTITUDE_CAUTIOUS:
				if (CvTeamAI::getTeam(getTeam()).getDefensivePower() < CvTeamAI::getTeam(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()).getPower())
				{
					return -MAX_INT + 1;
				}
				break;
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		CvPlayerAI& kOtherPlayer = CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer);

		int iDiploValue = 0;
		//if we like this player then value positive attitude, if however we really hate them then
		//actually value negative attitude.
		iDiploValue += ((iOtherPlayerAttitudeWeight + 50) * kEvent.getAttitudeModifier() * CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getPower()) / std::max(1, getPower());
		...
		if (kEvent.getTheirEnemyAttitudeModifier() != 0)
		{
			...
			{
			int iThirdPartyAttitudeWeight = CvTeamAI::getTeam(getTeam()).AI_getAttitudeWeight(eWorstEnemy);
		...
		if (kEvent.isDeclareWar())
		{
			int iWarValue = (CvTeamAI::getTeam(getTeam()).getDefensivePower() - CvTeamAI::getTeam(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()).getPower());// / std::max(1, CvTeamAI::getTeam(getTeam()).getDefensivePower());
...
void CvPlayerAI::AI_doNativeArmy(TeamTypes eTeam)
{
	...
	for (pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
	{
		...
		if (pLoopUnit->AI_getUnitAIType() == UNITAI_DEFENSIVE)
		{
			...
			if (pLoopCity != nullptr && pLoopCity->getOwnerINLINE() != getID())
			{
				pLoopCity = nullptr;
			}
			if (pLoopCity == nullptr || pLoopCity->AI_isDefended())
			{
...
int CvPlayerAI::AI_countNumHomedUnits(CvCity* pCity, UnitAITypes eUnitAI, UnitAIStates eUnitAIState)
{
	...
	for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
	{
		if (pCity == nullptr || pLoopUnit->getHomeCity() == pCity)
		{
...
void CvPlayerAI::AI_doSuppressRevolution()
{
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		{
			if (CvTeamAI::getTeam(getTeam()).isParentOf(kLoopPlayer.getTeam()))
			{
				if (atWar(getTeam(), kLoopPlayer.getTeam()))
				{
	...
	CvPlayerAI& kColony = CvPlayerAI::getPlayer(eColony);
	...
	if (AI_isStrategy(STRATEGY_CONCENTRATED_ATTACK) && AI_getStrategyData(STRATEGY_CONCENTRATED_ATTACK) == -1)
	{
		...
		int iLoop;
		CvCity* pLoopCity;
		for (pLoopCity = kColony.firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = kColony.nextCity(&iLoop))
		{
			if (pLoopCity->plot()->getNearestEurope() != NO_EUROPE)
			{
	...
	GC.getGameINLINE().getSorenRand().shuffleArray(shuffle, nullptr);
...
void CvPlayerAI::AI_doStrategy()
{
	...
	if (getParent() != NO_PLAYER)
	{
		...
		if (iGameTurn > GC.getGameINLINE().AI_adjustedTurn(100))
		{
			...
			int iRebelPercent = CvTeamAI::getTeam(getTeam()).getRebelPercent();
			...
			else
			{
				...
				if (AI_getStrategyDuration(STRATEGY_REVOLUTION_DECLARING) > GC.getGameINLINE().AI_adjustedTurn(20))
				{
					...
					if (iValue > 125)
					{
						if (AI_totalDefendersNeeded(nullptr) - AI_totalUnitAIs(UNITAI_OFFENSIVE) - AI_totalUnitAIs(UNITAI_COUNTER) - getNumCities() <= 0)
						{
	...
	if (isNative())
	{
		...
		else if (kTeam.getAnyWarPlanCount() > 0)
		{
			for (int i = 0; i < MAX_PLAYERS; ++i)
			{
				...
				{
					TeamTypes eLoopTeam = CvPlayerAI::getPlayer(eLoopPlayer).getTeam();
							...
							if ((bDefensiveBonuses && pLoopUnit->canDefend()) || pLoopUnit->canAttack())
							{
								if (!(pLoopUnit->isInvisible(getTeam(), false)))
								{
									if (pLoopUnit->atPlot(pPlot) || pLoopUnit->canMoveInto(*pPlot) || pLoopUnit->canMoveInto(*pPlot, /*bAttack*/ true))
									{
										if (!bTestMoves)
										{
											iValue += pLoopUnit->currEffectiveStr((bDefensiveBonuses ? pPlot : nullptr), nullptr);
										}
										else
										{
			...
			if (NO_UNIT != eUpgradeUnitType)
			{
				...
				if (iUpgradeValue > iUnitValue)
				{
					...
					if (pLoopUnit->upgradeAvailable(eUnitType, eUpgradeUnitClassType))
					{
						...
						if (pCapitalCity != nullptr && pCapitalCity->canTrain(eUpgradeUnitType))
						{
							bCanUpgrade = true;
						}
						else
						{
							...
							if (pCloseCity != nullptr && pCloseCity->canTrain(eUpgradeUnitType))
							{
								bCanUpgrade = true;
							}
						}
...
int CvPlayerAI::AI_playerCloseness(PlayerTypes eIndex, int iMaxDistance)
{
	...
	FAssert(CvPlayerAI::getPlayer(eIndex).isAlive());
	FAssert(eIndex != getID());

	iValue = 0;
	for (pLoopCity = CvPlayerAI::getPlayer(eIndex).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = CvPlayerAI::getPlayer(eIndex).nextCity(&iLoop))
	{
...
int CvPlayerAI::AI_totalDefendersNeeded(int* piUndefendedCityCount, CvArea* pArea, bool bIncludeStacks)
{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		if (pArea == nullptr || pArea == pLoopCity->area())
		{
	...
	if (bIncludeStacks)
	{
		for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
		{
			if (pLoopSelectionGroup->getNumUnits() > 1)
			{
				...
				{
					if (pArea == nullptr || pArea == pLoopSelectionGroup->area())
					{
						int iStackDefenders = pLoopSelectionGroup->countNumUnitAIType(UNITAI_DEFENSIVE) + AI_unitTargetMissionAIs(pLoopSelectionGroup->getHeadUnit(), MISSIONAI_GROUP, nullptr, UNITAI_DEFENSIVE);
	...
	if (piUndefendedCityCount != nullptr)
	{
	...
	for (iI = 0; iI < GC.getMap().numPlotsINLINE(); iI++)
	{
		...
		if ((pLoopPlot->area() == pArea) && pLoopPlot->isVisible(getTeam(), false) &&
			((bPlayer && pLoopPlot->getOwnerINLINE() == getID()) || (bTeam && pLoopPlot->getTeam() == getTeam())
				|| (bNeutral && !pLoopPlot->isOwned()) || (bHostile && pLoopPlot->isOwned() && CvTeamAI::getTeam(getTeam()).isAtWar(pLoopPlot->getTeam()))))
			{
			...
			if (iValue > iBestValue)
			{
				...
				if (eBuild != NO_BUILD)
				{
					...
					if (eImprovement != NO_IMPROVEMENT)
					{
						...
						if (pLoopPlot != nullptr && pLoopPlot->getImprovementType() != eImprovement)
						{
...
void CvPlayerAI::AI_advancedStartRouteTerritory()
{
	...
	{
		pLoopPlot = GC.getMap().plotByIndexINLINE(iI);
		if (pLoopPlot != nullptr && pLoopPlot->getOwner() == getID() && pLoopPlot->getRouteType() == NO_ROUTE)
		{
			if (pLoopPlot->getImprovementType() != NO_IMPROVEMENT)
			{
...
void CvPlayerAI::AI_doAdvancedStart(bool bNoExit)
{
	...
	if (!bIsNative)
	{
		...
		{
			for (int iPass = 0; iPass < 2 && getPrimaryCity() == nullptr; ++iPass)
			{
				CvPlot* pBestCapitalPlot = AI_advancedStartFindCapitalPlot();

				if (pBestCapitalPlot != nullptr)
				{
				...
				if (pLoopPlot->isRevealed(getTeam(), false))
				{
					...
					else
					{
						for (int iJ = 0; iJ < NUM_CARDINALDIRECTION_TYPES; iJ++)
						{
							...
							if (pLoopPlot2 != nullptr && getAdvancedStartVisibilityCost(true, pLoopPlot2) > 0)
							{
...
int CvPlayerAI::AI_bestCityUnitAIValue(UnitAITypes eUnitAI, CvCity* pCity, UnitTypes* peBestUnitType)
{
	...
	for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		...
		if (eLoopUnit != NO_UNIT)
		{
			...
			{
				if (pCity == nullptr ? canTrain(eLoopUnit) : pCity->canTrain(eLoopUnit))
				{
					iValue = AI_unitValue(eLoopUnit, eUnitAI, pCity == nullptr ? nullptr : pCity->area());
					if (iValue > iBestValue)
					{
						iBestValue = iValue;
						if (peBestUnitType != nullptr)
						{
				...
				if (pLoopUnit->canFight())
				{
					...
					if (atWar(getTeam(), pLoopUnit->getTeam()))
					{
						...
						else if (atWar(getTeam(), pLoopPlot->getTeam()))
						{
							...
							{
								iAdjacentAttackers = CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).AI_adjacentPotentialAttackers(pLoopPlot);
							}
...
int CvPlayerAI::AI_getPlotCanalValue(CvPlot* pPlot) const
{
	...
	if (pPlot == nullptr) return 0;//R&R mod, vetiarvind, super forts merge, add null check just in case
...
void CvPlayerAI::AI_diplomaticHissyFit(PlayerTypes ePlayer, int iAttitudeChange)
{
	...
	if (CvTeamAI::getTeam(getTeam()).AI_performNoWarRolls(kPlayer.getTeam()))
	{
		return;
	}
	...
	iProbability /= (CvTeamAI::getTeam(getTeam()).getNumMembers() * CvTeamAI::getTeam(kPlayer.getTeam()).getNumMembers());
	...
	CvTeamAI::getTeam(getTeam()).declareWar(kPlayer.getTeam(), true, WARPLAN_EXTORTION);
			...
			if (eLoopUnit != NO_UNIT)
			{
				...
				//if (kUnitInfo.getDefaultProfession() == NO_PROFESSION || kUnitInfo.getDefaultUnitAIType() == UNITAI_DEFENSIVE || kUnitInfo.getDefaultUnitAIType() == UNITAI_COUNTER)
				{
					...
					if ((iPrice > 0)  && (iPrice <= getGold()))
					{
						...
						const int iGoldValue = AI_unitGoldValue(eLoopUnit, eLoopUnitAI, nullptr);
...
bool CvPlayerAI::AI_isPathDanger(const CvSelectionGroup* pGroup, const CvPlot* pFromPlot, const CvPlot* pToPlot, int iRange) const
{
	...
	// fix: avoids nullptr crash when iMovesLeft is negative - Nightinggale
	// the issue is that iPathLength could become longer than getPathLength() resulting in nullptr plots being used without checking for nullptr
...
CvPlot* CvPlayerAI::AI_getBestWorkedYieldPlot(YieldTypes eYield)
{
	...
	//Automatically returns nullptr, if -1.
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (kLoopPlayer.isAlive())
		{
			if (!kLoopPlayer.isNative())
			{
				...
				{
					if (CvTeamAI::getTeam(getTeam()).isHasMet(kLoopPlayer.getTeam()))
					{
						int iLoop;
...
int CvPlayerAI::AI_getYieldBestExportPrice(YieldTypes eYield) const
{
	...
	const CvPlayer& kParent = CvPlayerAI::getPlayer(eParent);
