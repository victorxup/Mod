...
void CvSelectionGroup::kill()
{
	...
	CvPlayerAI::getPlayer(getOwnerINLINE()).removeGroupCycle(getID());

	CvPlayerAI::getPlayer(getOwnerINLINE()).deleteSelectionGroup(getID());
}
...
void CvSelectionGroup::doTurn()
{
	...
	if (getNumUnits() > 0)
	{
		...
		if (AI_isControlled())
		{
			if ((getActivityType() != ACTIVITY_MISSION) || (!canFight() && (CvPlayerAI::getPlayer(getOwnerINLINE()).AI_getPlotDanger(plot(), 1) > 0)))
			{
				setForceUpdate(true);
			}
		}
		else
		{
			...
			{
				if (CvPlayerAI::getPlayer(getOwnerINLINE()).AI_getPlotDanger(plot(), 1) > 0)
				{
					clearMissionQueue();
				}
			}
...
void CvSelectionGroup::autoMission()
{
	...
	if (getNumUnits() > 0)
	{
		if (headMissionQueueNode() != nullptr)
		{
			if (!isBusy())
			{
				if (isHuman() && CvPlayerAI::getPlayer(getOwnerINLINE()).AI_getPlotDanger(plot(), 1) > 0)
				{
					clearMissionQueue();
				}
				else
...
CvPlot* CvSelectionGroup::lastMissionPlot()
{
	...
	while (pMissionNode != nullptr)
	{
		...
		case MISSION_MOVE_TO_UNIT:
			pTargetUnit = CvPlayerAI::getPlayer((PlayerTypes)pMissionNode->m_data.iData1).getUnit(pMissionNode->m_data.iData2);
...
bool CvSelectionGroup::canStartMission(int iMission, int iData1, int iData2, CvPlot* pPlot, bool bTestVisible, bool bUseCache)
{
	...
	while (pUnitNode != nullptr)
	{
		...
		case MISSION_MOVE_TO_UNIT:
			...
			pTargetUnit = CvPlayerAI::getPlayer((PlayerTypes)iData1).getUnit(iData2);
			if (pTargetUnit != nullptr && !pTargetUnit->atPlot(pPlot))
			{
				return true;
			}
			break;
...
void CvSelectionGroup::startMission()
{
	...
	if (!GC.getGameINLINE().isMPOption(MPOPTION_SIMULTANEOUS_TURNS))
	{
		if (!CvPlayerAI::getPlayer(getOwnerINLINE()).isTurnActive())
		{
			if (getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
			{
				if (IsSelected())
...
//	bool bAction = false;
	...
	{
		FAssertMsg(CvPlayerAI::getPlayer(getOwnerINLINE()).isTurnActive() || CvPlayerAI::getPlayer(getOwnerINLINE()).isHuman(), "It's expected that either the turn is active for this player or the player is human");
		...
		if (!isBusy())
		{
			if (bDelete)
			{
				if (getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
				{
					if (IsSelected())
					{
						gDLL->getInterfaceIFace()->changeCycleSelectionCounter((CvPlayerAI::getPlayer(getOwnerINLINE()).isOption(PLAYEROPTION_QUICK_MOVES)) ? 1 : 2);
					}
				}
	...
	if (!bDone)
	{
		if (getNumUnits() > 0)
		{
			if (canAllMove())
			{
				...
				CLLNode<IDInfo>* pUnitNode = headUnitNode();
				bool bBreak = false;
				while (pUnitNode != nullptr && !bBreak)
				{
					CvUnit* pLoopUnit = ::getUnit(pUnitNode->m_data);
					pUnitNode = nextUnitNode(pUnitNode);
				...
				if (!bDone)
				{
					...
					case MISSION_MOVE_TO_UNIT:
						pTargetUnit = CvPlayerAI::getPlayer((PlayerTypes)headMissionQueueNode()->m_data.iData1).getUnit(headMissionQueueNode()->m_data.iData2);
						if (pTargetUnit != nullptr)
						{
...
bool CvSelectionGroup::canDoInterfaceMode(InterfaceModeTypes eInterfaceMode)
{
	...
	while (pUnitNode != nullptr)
	{
		...
		case INTERFACEMODE_GO_TO_ALL:
			if (getDomainType() != DOMAIN_IMMOBILE)
			{
				if (pLoopUnit->plot()->plotCount(nullptr, -1, -1, pLoopUnit->getOwnerINLINE()) > 1)
				{
					return true;
				}
			}
	...
	if (getNumUnits() > 0)
	{
		...
		if (pPlot != nullptr)
		{
			...
			while (pUnitNode != nullptr)
			{
				...
				if (pTransportUnit != nullptr && pTransportUnit->getGroup() == this)
				{
...
bool CvSelectionGroup::canAllMove()
{
	...
	if (getNumUnits() > 0)
	{
		...
		while (pUnitNode != nullptr)
		{
			...
			FAssertMsg(pLoopUnit != nullptr, "existing node, but null unit");

			if (pLoopUnit != nullptr && !(pLoopUnit->canMove()))
			{
				return false;
			}
			pUnitNode = nextUnitNode(pUnitNode);
...
{
	// K-Mod. (hack to avoid breaking the __declspec(dllexport)) advc: 2x const, CvPlot&
	...
	bool bCheckMoves, bool bAssumeVisible) const // K-Mod
{
	...
	for (CLLNode<IDInfo> const* pUnitNode = headUnitNode(); pUnitNode != nullptr;
		pUnitNode = nextUnitNode(pUnitNode))
	{
...
int CvSelectionGroup::getBombardTurns(CvCity* pCity)
{
	...
	//bool bHasBomber = (getOwnerINLINE() != NO_PLAYER ? (CvPlayerAI::getPlayer(getOwnerINLINE()).AI_calculateTotalBombard(DOMAIN_AIR) > 0) : false);
...
int CvSelectionGroup::countNumUnitAIType(UnitAITypes eUnitAI)
{
	...
	FAssertMsg(headUnitNode() != nullptr, "headUnitNode() is not expected to be null");
...
bool CvSelectionGroup::groupAttack(int iX, int iY, int iFlags, bool& bFailedAlreadyFighting)
{
	...
	bool bStack = (isHuman() && CvPlayerAI::getPlayer(getOwnerINLINE()).isOption(PLAYEROPTION_STACK_ATTACK));
	bool bAttack = false;
...
void CvSelectionGroup::groupMove(CvPlot* pPlot, bool bCombat, CvUnit* pCombatUnit, bool bEndMove)
{
	...
	{
		if (pCombatUnit == nullptr || pUnitNode->m_data != pCombatUnit->getIDInfo())
		{
...
bool CvSelectionGroup::groupPathTo(int iX, int iY, int iFlags)
{
	...
	*/
	...
	//groupMove(pPathPlot, iFlags & MOVE_THROUGH_ENEMY, nullptr, bEndMove);
	groupMove(pPathPlot, false, nullptr, bEndMove); // K-Mod
...
void CvSelectionGroup::setTransportUnit(CvUnit* pTransportUnit)
{
	...
	{
		CvUnit* pHeadUnit = getHeadUnit();
		FAssertMsg(pHeadUnit != nullptr, "non-zero group without head unit");
...
{
	return (bAny ? canAnyMove() : canAllMove()) && headMissionQueueNode() == nullptr && getActivityType() == ACTIVITY_AWAKE && !isBusy() && !isCargoBusy();
}


bool CvSelectionGroup::readyToAuto()
{
	return canAllMove() && headMissionQueueNode() != nullptr;
}
...
void CvSelectionGroup::updateMissionTimer(int iSteps)
{
	...
	else if (headMissionQueueNode() != nullptr)
	{
		...
		if (isHuman() && (isAutomated() || (CvPlayerAI::getPlayer((GC.getGameINLINE().isNetworkMultiPlayer()) ? getOwnerINLINE() : GC.getGameINLINE().getActivePlayer()).isOption(PLAYEROPTION_QUICK_MOVES))))
		{
...
int CvSelectionGroup::getPathLength() const
{
	...
	if (pNode != nullptr)
	{
		...
		while (pNode != nullptr)
		{
			...
			if (pNode->m_pParent->m_pParent == nullptr)
			{
				return iLength;
			}
...
bool CvSelectionGroup::generatePath(const CvPlot* pFromPlot, const CvPlot* pToPlot, int iFlags, bool bReuse, int* piPathTurns, int iMaxPath) const
{
	...
	if (pFromPlot == nullptr || pToPlot == nullptr)
		return false;
	...
	//::MessageBoxA(nullptr,"pathfind mismatch","CvGameCore",MB_OK);
	FAssert(false);
	}
	}*/
	...
	if (bSuccess)
	{
		if (CvPlayerAI::getPlayer(getOwnerINLINE()).AI_needsProtection(getHeadUnitAI()))
		{
			if ((iFlags & MOVE_IGNORE_DANGER)== 0)
			{
				if (!bIgnoreDanger || plot()->getPlotCity() != nullptr)
				{
					if (getPathEndTurnPlot() != pToPlot)
					{
						if (CvPlayerAI::getPlayer(getOwnerINLINE()).AI_isPathDanger(this, pFromPlot, pToPlot))
						{
							bSuccess = false;
						}
					}
...
void CvSelectionGroup::mergeIntoGroup(CvSelectionGroup* pSelectionGroup)
{
	...
	do
	{
		...
		CLLNode<IDInfo>* pUnitNode = headUnitNode();
		while (pUnitNode != nullptr && !bChangedUnitAI)
		{
			CvUnit* pLoopUnit = ::getUnit(pUnitNode->m_data);
			pUnitNode = nextUnitNode(pUnitNode);
			...
			if (pLoopUnit != nullptr)
			{
				...
				if (pNewHeadUnit != nullptr && eUnitAI != eNewHeadUnitAI && pLoopUnit->AI_groupFirstVal() > pNewHeadUnit->AI_groupFirstVal())
				{
					// non-zero AI_unitValue means that this UnitAI is valid for this unit (that is the check used everywhere)
					if (kPlayer.AI_unitValue(pLoopUnit->getUnitType(), eNewHeadUnitAI, nullptr) > 0)
					{
...
CvSelectionGroup* CvSelectionGroup::splitGroup(int iSplitSize, CvUnit* pNewHeadUnit)
{
	...
	FAssertMsg(pOldHeadUnit != nullptr, "non-zero group without head unit");
	if (pOldHeadUnit == nullptr)
	{
		return nullptr;
	}
	...
	// if pNewHeadUnit nullptr, then we will use our current head to head the new split group of target size
	if (pNewHeadUnit == nullptr)
	{
	...
	CvUnit* pRemainderHeadUnit = nullptr;
	...
	if (pRemainderHeadUnit == nullptr)
	{
		// loop over all the units
		pUnitNode = headUnitNode();
		while (pUnitNode != nullptr && pRemainderHeadUnit == nullptr)
		{
			CvUnit* pLoopUnit = ::getUnit(pUnitNode->m_data);
			pUnitNode = nextUnitNode(pUnitNode);

			if (pLoopUnit != nullptr && pLoopUnit != pNewHeadUnit)
			{
	...
	CvSelectionGroup* pSplitGroup = nullptr;
	CvSelectionGroup* pRemainderGroup = nullptr;

	// make the new group for the new head
	pNewHeadUnit->joinGroup(nullptr);
	pSplitGroup = pNewHeadUnit->getGroup();
	FAssertMsg(pSplitGroup != nullptr, "join resulted in null group");

	// make a new group for the remainder, if non-null
	if (pRemainderHeadUnit != nullptr)
	{
		pRemainderHeadUnit->joinGroup(nullptr);
		pRemainderGroup = pRemainderHeadUnit->getGroup();
		FAssertMsg(pRemainderGroup != nullptr, "join resulted in null group");
	}
	...
	while (getNumUnits())
	{
		...
		while (pUnitNode != nullptr)
		{
			...
			if (pLoopUnit != nullptr)
			{
				...
				{
					// move this unit to the appropriate group (if pRemainderGroup nullptr, it gets its own group)
...
CLLNode<MissionData>* CvSelectionGroup::deleteMissionQueueNode(CLLNode<MissionData>* pNode)
{
	...
	FAssertMsg(pNode != nullptr, "Node is not assigned a valid value");
	FAssert(getOwnerINLINE() != NO_PLAYER);
...
{
	if (CvPlayerAI::getPlayer(getOwnerINLINE()).getTradeRoute(iRouteID) == nullptr)
	{
...
int CvSelectionGroup::maxMoves() const
{
	...
	for (CLLNode<IDInfo> const* pUnitNode = headUnitNode(); pUnitNode != nullptr; pUnitNode = nextUnitNode(pUnitNode))
	{
		CvUnit const* pLoopUnit = ::getUnit(pUnitNode->m_data);
