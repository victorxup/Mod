...
// WARNING: plotINLINE can return nullptr. Never use such a pointer before checking if it's nullptr first.
// nullptr only happens if the plot is outside the map, but as a general rule the game shouldn't crash if that happens.
//   Nightinggale
