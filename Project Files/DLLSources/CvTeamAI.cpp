...
CvTeamAI* CvTeamAI::m_aTeams = nullptr;
...
int CvTeamAI::AI_getOurPlotStrength(CvPlot* pPlot, int iRange, bool bDefensiveBonuses, bool bTestMoves, bool bIncludeVassals) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iPlotStrength += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_getOurPlotStrength(pPlot,iRange,bDefensiveBonuses,bTestMoves);
			}
		}
	}
...
void CvTeamAI::AI_updateAreaTargets()
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				CvPlayerAI::getPlayer((PlayerTypes)iI).AI_updateAreaTargets();
			}
		}
	}
}
...
int CvTeamAI::AI_countMilitaryWeight(CvArea* pArea) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iCount += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_militaryWeight(pArea);
			}
		}
	}
...
bool CvTeamAI::AI_isAnyCapitalAreaAlone() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).AI_isCapitalAreaAlone())
				{
					return true;
				}
			}
...
bool CvTeamAI::AI_isPrimaryArea(CvArea* pArea) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).AI_isPrimaryArea(pArea))
				{
					return true;
				}
			}
...
bool CvTeamAI::AI_hasCitiesInPrimaryArea(TeamTypes eTeam) const
{
	...
	for (pLoopArea = GC.getMap().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMap().nextArea(&iLoop))
	{
		...
		{
			if (CvTeamAI::getTeam(eTeam).countNumCitiesByArea(pLoopArea))
			{
				return true;
			}
		}
		...
		for (iI = 0; iI < MAX_TEAMS; iI++)
		{
			if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
			{
				...
				if (eWarplan != NO_WARPLAN)
				{
					...
					if ((CvTeamAI::getTeam((TeamTypes)iI).countNumCitiesByArea(pArea) > 0) || (CvTeamAI::getTeam((TeamTypes)iI).countNumUnitsByArea(pArea) > 4))
					{
...
int CvTeamAI::AI_calculateCapitalProximity(TeamTypes eTeam) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				pOurCapitalCity = CvPlayerAI::getPlayer((PlayerTypes)iI).getPrimaryCity();

				if (pOurCapitalCity != nullptr)
				{
					for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
						{
							if (CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam() != getID())
							{
								pTheirCapitalCity = CvPlayerAI::getPlayer((PlayerTypes)iJ).getPrimaryCity();

								if (pTheirCapitalCity != nullptr)
								{
...
bool CvTeamAI::AI_shareWar(TeamTypes eTeam) const
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
		{
			...
			{
				if (isAtWar((TeamTypes)iI) && CvTeamAI::getTeam(eTeam).isAtWar((TeamTypes)iI))
				{
					// R&R, ray, fix for improved Attitude through shared war with Animals
					if(iI != CvPlayerAI::getPlayer(GC.getGameINLINE().getBarbarianPlayer()).getTeam())
					{
						return true;
					}
				}
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive() && iI != iJ)
					{
						TeamTypes eTeamLoop = CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam();
						if (eTeamLoop == eTeam)
						{
							iAttitude += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_getAttitude((PlayerTypes)iJ, bForced);
							iCount++;
						}
					}
				}
			...
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam() == eTeam)
						{
							iAttitudeVal += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_getAttitudeVal((PlayerTypes)iJ, bForced);
							iCount++;
						}
					}
				}
			...
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam() == eTeam)
						{
							iMemoryCount += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_getMemoryCount(((PlayerTypes)iJ), eMemory);
							iCount++;
						}
					}
				}
...
int CvTeamAI::AI_endWarVal(TeamTypes eTeam) const
{
	...
	iValue += (CvTeamAI::getTeam(eTeam).getNumCities() * 2);

	iValue += getTotalPopulation();
	iValue += CvTeamAI::getTeam(eTeam).getTotalPopulation();

	iValue += (CvTeamAI::getTeam(eTeam).AI_getWarSuccess(getID()) * 10);

	iValue += CvTeamAI::getTeam(eTeam).AI_getDamages(getID());

	int iOurPower = std::max(1, getPower());
	int iTheirPower = std::max(1, CvTeamAI::getTeam(eTeam).getDefensivePower());
	...
	if (CvTeamAI::getTeam(eTeam).AI_isNative())
	{
		if (CvTeamAI::getTeam(eTeam).AI_isChosenWar(getID()))
		{
			iValue *= 25 + CvTeamAI::getTeam(eTeam).AI_targetValidity(getID());
			iValue /= 125;
		}
	}
	...
	if ((!(isHuman()) && (eWarPlan == WARPLAN_TOTAL)) ||
		  (!(CvTeamAI::getTeam(eTeam).isHuman()) && (CvTeamAI::getTeam(eTeam).AI_getWarPlan(getID()) == WARPLAN_TOTAL)))
	{
		iValue *= 2;
	}
	else if ((!(isHuman()) && (eWarPlan == WARPLAN_DOGPILE) && (CvTeamAI::getTeam(eTeam).getAtWarCount(true) > 1)) ||
		       (!(CvTeamAI::getTeam(eTeam).isHuman()) && (CvTeamAI::getTeam(eTeam).AI_getWarPlan(getID()) == WARPLAN_DOGPILE) && (getAtWarCount(true) > 1)))
	{
		iValue *= 3;
		iValue /= 2;
	}*/
...
DenialTypes CvTeamAI::AI_mapTrade(TeamTypes eTeam) const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getMapRefuseAttitudeThreshold())
				{
...
int CvTeamAI::AI_makePeaceTradeVal(TeamTypes ePeaceTeam, TeamTypes eTeam) const
{
	...
	FAssertMsg(CvTeamAI::getTeam(ePeaceTeam).isAlive(), "CvTeamAI::getTeam(ePeaceTeam).isAlive is expected to be true");
	FAssertMsg(atWar(ePeaceTeam, eTeam), "eTeam should be at war with ePeaceTeam");

	iValue = (50 + GC.getGameINLINE().getGameTurn());
	iValue += ((CvTeamAI::getTeam(eTeam).getNumCities() + CvTeamAI::getTeam(ePeaceTeam).getNumCities()) * 8);

	iModifier = 0;

	switch ((CvTeamAI::getTeam(eTeam).AI_getAttitude(ePeaceTeam) + CvTeamAI::getTeam(ePeaceTeam).AI_getAttitude(eTeam)) / 2)
	{
	case ATTITUDE_FURIOUS:
		iModifier += 400;
		break;
	...
	iValue *= 40;
	iValue /= (CvTeamAI::getTeam(eTeam).AI_getAtWarCounter(ePeaceTeam) + 10);
...
DenialTypes CvTeamAI::AI_makePeaceTrade(TeamTypes ePeaceTeam, TeamTypes eTeam) const
{
	...
	if (CvTeamAI::getTeam(ePeaceTeam).isHuman())
	{
	...
	if (isInRevolution() && CvTeamAI::getTeam(ePeaceTeam).hasEuropePlayer())
	{
	...
	if (AI_endWarVal(ePeaceTeam) > (CvTeamAI::getTeam(ePeaceTeam).AI_endWarVal(getID()) * 2))
	{
		return DENIAL_CONTACT_THEM;
	}

    int iLandRatio = ((getTotalLand() * 100) / std::max(20, CvTeamAI::getTeam(eTeam).getTotalLand()));
    if (iLandRatio > 250)
    {
		return DENIAL_VICTORY;
	}
...
int CvTeamAI::AI_declareWarTradeVal(TeamTypes eWarTeam, TeamTypes eTeam) const
{
	...
	FAssertMsg(CvTeamAI::getTeam(eWarTeam).isAlive(), "CvTeamAI::getTeam(eWarTeam).isAlive is expected to be true");
	FAssertMsg(!atWar(eWarTeam, eTeam), "eTeam should be at peace with eWarTeam");

	iValue = 0;
	iValue += (CvTeamAI::getTeam(eWarTeam).getNumCities() * 10);
	iValue += (CvTeamAI::getTeam(eWarTeam).getTotalPopulation() * 2);

	iModifier = 0;

	if (CvTeamAI::getTeam(eTeam).AI_isNative() && CvTeamAI::getTeam(eWarTeam).AI_isKing())
	{
		return 0;
	}

	switch (CvTeamAI::getTeam(eTeam).AI_getAttitude(eWarTeam))
	{
	case ATTITUDE_FURIOUS:
		break;
	...
	int iTheirPower = CvTeamAI::getTeam(eTeam).getPower();
	int iWarTeamPower = CvTeamAI::getTeam(eWarTeam).getPower();

	iValue *= 50 + ((100 * iWarTeamPower) / (iTheirPower + iWarTeamPower + 1));
	iValue /= 100;

	if (!(CvTeamAI::getTeam(eTeam).AI_isAllyLandTarget(eWarTeam)))
	{
		iValue *= 2;
	}
	...
	iValue += (CvTeamAI::getTeam(eTeam).getNumCities() * 20);
	iValue += (CvTeamAI::getTeam(eTeam).getTotalPopulation() * 15);

	if (isAtWar(eWarTeam))
	{
		switch (CvTeamAI::getTeam(eTeam).AI_getAttitude(getID()))
		{
		case ATTITUDE_FURIOUS:
		case ATTITUDE_ANNOYED:
		case ATTITUDE_CAUTIOUS:
	...
	if (CvTeamAI::getTeam(eWarTeam).getAtWarCount() == 0)
	{
		iValue *= 2;
		...
		for (int iI = 0; iI < MAX_TEAMS; iI++)
		{
			if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
			{
				...
				{
					if (CvTeamAI::getTeam(eWarTeam).isDefensivePact((TeamTypes)iI))
					{
						iValue += (CvTeamAI::getTeam((TeamTypes)iI).getNumCities() * 30);
						iValue += (CvTeamAI::getTeam((TeamTypes)iI).getTotalPopulation() * 20);
					}
				}
			}
		}
...
DenialTypes CvTeamAI::AI_declareWarTrade(TeamTypes eWarTeam, TeamTypes eTeam, bool bConsiderPower) const
{
	...
	if (CvTeamAI::getTeam(eWarTeam).isDefensivePact(eTeam))
	{
	...
	if (AI_isNative())
	{
		if (CvTeamAI::getTeam(eWarTeam).isParentOf(eTeam))
		{
	...
	if (bConsiderPower)
	{
		...
		if ((CvTeamAI::getTeam(eWarTeam).getDefensivePower() / ((bLandTarget) ? 2 : 1)) >
			(getPower() + ((atWar(eWarTeam, eTeam)) ? CvTeamAI::getTeam(eTeam).getPower() : 0)))
		{
			if (bLandTarget)
			{
	...
	{
		CvTeamAI& kWarTeam = CvTeamAI::getTeam(eWarTeam);
	...
	if (!bConcerned)
	{
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
				{
					if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getDeclareWarRefuseAttitudeThreshold())
					{
						return DENIAL_ATTITUDE;
					}
				}
	...
	if (!bVeryConcerned)
	{
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
				{
					if (eAttitudeThem > GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getDeclareWarThemRefuseAttitudeThreshold())
					{
...
{
	return (getNumCities() + CvTeamAI::getTeam(eTeam).getNumCities());
}
...
DenialTypes CvTeamAI::AI_openBordersTrade(TeamTypes eTeam) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getOpenBordersRefuseAttitudeThreshold())
				{
					return DENIAL_ATTITUDE;
				}
			}
...
{
	return ((getNumCities() + CvTeamAI::getTeam(eTeam).getNumCities()) * 3);
}
...
DenialTypes CvTeamAI::AI_defensivePactTrade(TeamTypes eTeam) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold())
				{
					return DENIAL_ATTITUDE;
				}
			}
...
DenialTypes CvTeamAI::AI_permanentAllianceTrade(TeamTypes eTeam) const
{
	...
	if ((getPower() + CvTeamAI::getTeam(eTeam).getPower()) > (GC.getGameINLINE().countTotalCivPower() / 2))
	{
		if (getPower() > CvTeamAI::getTeam(eTeam).getPower())
		{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				if (eAttitude <= GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getPermanentAllianceRefuseAttitudeThreshold())
				{
					return DENIAL_ATTITUDE;
				}
			}
	...
	if (!CvTeamAI::getTeam(eTeam).isInRevolution() && GC.getDefineINT("PERMANENT_ALLIANCE_REQUIRES_REVOLUTION"))
	{
		return DENIAL_NO_GAIN;
	}
...
void CvTeamAI::AI_updateWorstEnemy()
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
		{
			...
			if (iI != getID() && (TeamTypes)iI != CvPlayerAI::getPlayer(GC.getGameINLINE().getBarbarianPlayer()).getTeam())
			{
	...
	if (AI_getWarPlan(eIndex) != eNewValue)
	{
		if (bWar || !isAtWar(eIndex))
		{
			...
			if (gTeamLogLevel >= 1)
			{
				...
				logBBAI("Team %d (%S) CvTeamAI::AI_setWarPlan (%S) against team %d (%S)", getID(), CvPlayerAI::getPlayer(getLeaderID()).getCivilizationDescription(0), buf.getCString(), eIndex, CvPlayerAI::getPlayer(CvTeamAI::getTeam(eIndex).getLeaderID()).getCivilizationDescription(0));
			}
				...
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
					{
						if (!(CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman()))
						{
							...
							{
								CvPlayerAI::getPlayer((PlayerTypes)iI).AI_doNativeArmy(eIndex);
							}
						}
					}
				}
		...
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam() == eIndex)
						{
							iValue += CvPlayerAI::getPlayer((PlayerTypes)iI).AI_playerCloseness((PlayerTypes)iJ, iMaxDistance);
						}
					}
				}
			}
			...
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					if (CvPlayerAI::getPlayer((PlayerTypes)iJ).isAlive())
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iJ).getTeam() == eTeam)
						{
							iValue = std::max(iValue, CvPlayerAI::getPlayer((PlayerTypes)iI).AI_targetValidity((PlayerTypes)iJ));
						}
					}
				}
			}
	...
	bool bGarrisonStrength) const // advc.500b
{
	...
	though. */
	if (kPlot.getOwner() != NO_PLAYER && CvTeamAI::getTeam(getID()).isAtWar(kPlot.getTeam()))
...
int CvTeamAI::AI_maxWarRand() const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		CvPlayerAI& kPlayer = CvPlayerAI::getPlayer((PlayerTypes)iI);
		if (kPlayer.isAlive())
		{
			if (kPlayer.getTeam() == getID())
			{
				if (kPlayer.isNative())
				{
					...
					int iTheirPop = CvTeamAI::getTeam(kPlayer.getTeam()).getTotalPopulation();
...
int CvTeamAI::AI_maxWarDistantPowerRatio() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getMaxWarDistantPowerRatio();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_maxWarMinAdjacentLandPercent() const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getMaxWarMinAdjacentLandPercent();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_limitedWarRand() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getLimitedWarRand();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_limitedWarPowerRatio() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getLimitedWarPowerRatio();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_dogpileWarRand() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getDogpileWarRand();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_makePeaceRand() const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iRand += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getMakePeaceRand();
				iCount++;
			}
		}
	}
...
int CvTeamAI::AI_noWarAttitudeProb(AttitudeTypes eAttitude) const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				iProb += GC.getLeaderHeadInfo(CvPlayerAI::getPlayer((PlayerTypes)iI).getPersonalityType()).getNoWarAttitudeProb(eAttitude);
				iCount++;
			}
		}
	}
...
void CvTeamAI::AI_doWar()
{
		...
		if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
		{
			...
			if (eWarPlan != NO_WARPLAN)
			{
				int iTimeModifier = 100;
				{
					...
					int iTheirPower = CvTeamAI::getTeam((TeamTypes)iI).getPower();
				...
				if (eWarPlan == WARPLAN_ATTACKED_RECENT)
				{
					...
					if (AI_getAtWarCounter((TeamTypes)iI) > ((CvTeamAI::getTeam((TeamTypes)iI).AI_isLandTarget(getID())) ? 9 : 3))
					{
				...
				else if (eWarPlan == WARPLAN_PREPARING_TOTAL)
				{
					...
					if (AI_getWarPlanStateCounter((TeamTypes)iI) > ((10 * iTimeModifier) / 100))
					{
						...
						for (pLoopArea = GC.getMap().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMap().nextArea(&iLoop))
						{
							if (AI_isPrimaryArea(pLoopArea))
							{
								if (CvTeamAI::getTeam((TeamTypes)iI).countNumCitiesByArea(pLoopArea) > 0)
								{
									bShareValid = true;
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getID())
			{
				CvPlayerAI::getPlayer((PlayerTypes)iI).AI_doPeace();
			}
		}
	}
							...
							if (iI != getID())
							{
								if (isHasMet((TeamTypes)iI))
								{
									if (canDeclareWar((TeamTypes)iI))
									{
										...
										{
											int iDefensivePower = (CvTeamAI::getTeam((TeamTypes)iI).getDefensivePower() * 2) / 3;
						...
						if (iI != getID())
						{
							if (isHasMet((TeamTypes)iI))
							{
								if (canDeclareWar((TeamTypes)iI))
								{
									if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)))
									{
										if (AI_isLandTarget((TeamTypes)iI) || (AI_isAnyCapitalAreaAlone() && CvTeamAI::getTeam((TeamTypes)iI).AI_isAnyCapitalAreaAlone()))
										{
											if (CvTeamAI::getTeam((TeamTypes)iI).getDefensivePower() < ((iOurPower * AI_limitedWarPowerRatio()) / 100))
											{
												iValue = AI_startWarVal((TeamTypes)iI);
						...
						if (iI != getID())
						{
							if (isHasMet((TeamTypes)iI))
							{
								if (canDeclareWar((TeamTypes)iI))
								{
									if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)))
									{
										if (CvTeamAI::getTeam((TeamTypes)iI).getAtWarCount() > 0)
										{
											if (AI_isLandTarget((TeamTypes)iI))
											{
												...
												for (iJ = 0; iJ < MAX_TEAMS; iJ++)
												{
													if (CvTeamAI::getTeam((TeamTypes)iJ).isAlive())
													{
														if (iJ != iI)
														{
															...
															{
																iDogpilePower += CvTeamAI::getTeam((TeamTypes)iJ).getPower();
															}
														}
													}
												}

												FAssert(CvTeamAI::getTeam((TeamTypes)iI).getPower() == CvTeamAI::getTeam((TeamTypes)iI).getDefensivePower());

												if (((CvTeamAI::getTeam((TeamTypes)iI).getDefensivePower() * 3) / 2) < iDogpilePower)
												{
													iValue = AI_startWarVal((TeamTypes)iI);
...
//	CvArea* pLoopArea;
//	int iLoop;
//	for (pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
//	{
...
void CvTeamAI::AI_doTactics()
{
	...
	for(int iI = 0; iI < kMap.numPlotsINLINE(); iI++)
	{
		...
		if ((pLoopPlot->isCity() && (AI_getWarPlan(pLoopPlot->getTeam()) != NO_WARPLAN))
			&& !(CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).isNative() && AI_isKing()))
		{
...
{
	CvTeamAI& kOtherTeam = CvTeamAI::getTeam(eTeam);
	int iValue = 100;
