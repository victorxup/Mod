...
CvPlayer::~CvPlayer()
{
	uninit();
	// cache CvPlayer::getYieldEquipmentAmount - start - Nightinggale
	if (m_cache_YieldEquipmentAmount != nullptr)
	{
...
void CvPlayer::init(PlayerTypes eID)
{
	...
	{
		CvPlayer& kParent = CvPlayerAI::getPlayer((PlayerTypes) iParent);
	...
	CvTeamAI::getTeam(getTeam()).changeNumMembers(1);
			...
			if (eLoopUnit != NO_UNIT)
			{
				...
				for (int iJ = 0; iJ < iFreeCount; iJ++)
				{
					...
					if (GC.getCivilizationInfo(getCivilizationType()).isEurope())
					{
						...
						{
							CvPlayer& kChild = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
...
void CvPlayer::addFreeUnitAI(UnitAITypes eUnitAI, int iCount)
{
	...
	for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		...
		if (eLoopUnit != NO_UNIT)
		{
			...
			{
				int iValue = AI_unitValue(eLoopUnit, eUnitAI, nullptr);
...
//	iRange = (GC.getMapINLINE().maxStepDistance() + 40);
	...
	long lResult=0;
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "minStartingDistanceModifier", nullptr, &lResult))
	{
...
int CvPlayer::findStartingArea() const
{
	...
	{
		if (result == -1 || GC.getMap().getArea(result) != nullptr)
		{
			return result;
		}
		else
...
CvPlot* CvPlayer::findStartingPlot(bool bRandomize)
{
	...
	bool bNew = false;
	if (getStartingPlot() != nullptr)
	{
		iBestArea = getStartingPlot()->getArea();
		setStartingPlot(nullptr, true);
		bNew = true;
	}
			...
			if ((iBestArea == -1) || (pLoopPlot->getArea() == iBestArea) || (iPass > 0))
			{
				if (bStartOnWater)
				{
					...
					if (eEurope != NO_EUROPE && abEuropeStart[eEurope])
					{
						...
						for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
						{
							CvPlayer& otherPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bTrade)
{
						...
						if (pLoopPlot->getOwnerINLINE() == pOldCity->getOwnerINLINE())
						{
							if (pLoopPlot->getNumCultureRangeCities(pOldCity->getOwnerINLINE()) == 1)
							{
								...
								for (iI = 0; iI < MAX_PLAYERS; iI++)
								{
									...
									{
										if ((CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() != getTeam()) && (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() != pOldCity->getTeam()))
										{
	...
	{
		CvPlayerAI::getPlayer(pOldCity->getOriginalOwner()).changeCitiesLost(1);
	}
	...
	if (bConquest)
	{
		...
		szName.Format(L"%s (%s)", pOldCity->getName().GetCString(), CvPlayerAI::getPlayer(pOldCity->getOwnerINLINE()).getName());
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
			{
				if (iI != getID())
				{
					if (pOldCity->isRevealed(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam(), false))
					{
	...
	for (int i = 0; i < iPopulation; ++i)
	{
		...
		CvPlayerAI::getPlayer(pOldCity->getOwnerINLINE()).getAndRemoveUnit(pUnit->getID());
	}
	FAssert(pOldCity->getPopulation() == 0);

	bRecapture = ((eHighestCulturePlayer != NO_PLAYER) ? (CvPlayerAI::getPlayer(eHighestCulturePlayer).getTeam() == getTeam()) : false);
	...
	FAssertMsg(pNewCity != nullptr, "NewCity is not assigned a valid value");
	...
	pCityPlot->setRevealed(CvPlayerAI::getPlayer(eOldOwner).getTeam(), true, false, NO_TEAM);
	...
	if (!bTrade)
	{
		...
		{
			eOldPreviousTeam = CvPlayerAI::getPlayer(eOldPreviousOwner).getTeam();
		}
		...
		if (lResult == 1)
		{
			...
			else
			{
				...
				bGift = ((eHighestCulturePlayer != NO_PLAYER)
						...
						&& ((getTeam() == CvPlayerAI::getPlayer(eHighestCulturePlayer).getTeam())
							|| CvTeamAI::getTeam(getTeam()).isOpenBorders(CvPlayerAI::getPlayer(eHighestCulturePlayer).getTeam())));
...
void CvPlayer::killCities()
{
	...
	for (int iI = 0; iI < GC.getMap().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot != nullptr && pLoopPlot->getOwner() == ePlayer) //R&R mod, vetiarvind, super forts merge - added null check
		{
...
bool CvPlayer::isCityNameValid(const CvWString& szName, bool bTestDestroyed) const
{
	...
	if (bTestDestroyed)
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			for (pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = kLoopPlayer.nextCity(&iLoop))
			{
...
CvUnit* CvPlayer::initUnit(UnitTypes eUnit, ProfessionTypes eProfession, Coordinates initCoord, UnitAITypes eUnitAI, DirectionTypes eFacingDirection, int iYieldStored)
{
	...
	FAssertMsg(pUnit != nullptr, "Unit is not assigned a valid value");
	if (pUnit != nullptr)
	{
		...
		{
			if (gDLL->getInterfaceIFace()->getHeadSelectedUnit() == nullptr)
			{
...
CvSelectionGroup* CvPlayer::cycleSelectionGroups(CvUnit* pUnit, bool bForward, bool* pbWrap)
{
	...
	if (pUnit != nullptr)
	{
		while (pSelectionGroupNode != nullptr)
		{
	...
	if (pSelectionGroupNode == nullptr)
	{
		if (bForward)
		{
			pSelectionGroupNode = headGroupCycleNode();
	...
	if (pSelectionGroupNode != nullptr)
	{
		...
		while (true)
		{
			...
			FAssertMsg(pLoopSelectionGroup != nullptr, "LoopSelectionGroup is not assigned a valid value");
...
const CvWString CvPlayer::getWorstEnemyName() const
{
	...
	eWorstEnemy = CvTeamAI::getTeam(getTeam()).AI_getWorstEnemy();

	if (eWorstEnemy != NO_TEAM)
	{
		return CvTeamAI::getTeam(eWorstEnemy).getName();
	}
...
void CvPlayer::doTurnUnits()
{
	...
	for (pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup != nullptr; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
	...
	{
		CvPlayer& kEurope = CvPlayerAI::getPlayer(getParent());
		if (kEurope.isAlive() && kEurope.isEurope() && !::atWar(getTeam(), kEurope.getTeam()))
		{
			...
			if (eCheapestShip != NO_UNIT && getGold() < getEuropeUnitBuyPrice(eCheapestShip))
			{
				...
				bool bHasShip = false;
				int iLoop;
				for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
				{
					// TAC - AI Improved Navel AI - koma13 - START
...
int CvPlayer::calculateScore(bool bFinal, bool bVictory) const
{
	...
	if (CvTeamAI::getTeam(getTeam()).getNumMembers() == 0)
	{
		return 0;
	}
...
bool CvPlayer::canContact(PlayerTypes ePlayer) const
{
	...
	if (!isAlive() || !(CvPlayerAI::getPlayer(ePlayer).isAlive()))
	{
		return false;
	}

	if (getTeam() != CvPlayerAI::getPlayer(ePlayer).getTeam())
	{
		if (!(CvTeamAI::getTeam(getTeam()).isHasMet(CvPlayerAI::getPlayer(ePlayer).getTeam())))
		{
			return false;
		}

		if (atWar(getTeam(), CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
			if (!(CvTeamAI::getTeam(getTeam()).canChangeWarPeace(CvPlayerAI::getPlayer(ePlayer).getTeam())))
			{
				return false;
			}

			if (isHuman() || CvPlayerAI::getPlayer(ePlayer).isHuman())
			{
			...
			if (CvTeamAI::getTeam(getTeam()).isParentOf(CvPlayerAI::getPlayer(ePlayer).getTeam()) || CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).isParentOf(getTeam()))
			{
				return false;
			}
		}
		...
		/*
		if (CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).isParentOf(getTeam()))
		{
		...
		if (CvTeamAI::getTeam(getTeam()).isParentOf(CvPlayerAI::getPlayer(ePlayer).getTeam()))
		{
			if (CvPlayerAI::getPlayer(ePlayer).getParent() != getID())
			{
				return false;
			}
		}

		if (isEurope() && CvPlayerAI::getPlayer(ePlayer).getParent() != getID())
		{
			return false;
		}

		if (CvPlayerAI::getPlayer(ePlayer).isEurope() && getParent() != ePlayer)
		{
			return false;
		}
		*/
...
void CvPlayer::contact(PlayerTypes ePlayer)
{
	...
	{
		if (GC.getGameINLINE().isPbem() || GC.getGameINLINE().isHotSeat() || (GC.getGameINLINE().isPitboss() && !gDLL->isConnected(CvPlayerAI::getPlayer(ePlayer).getNetID())))
		{
...
{
	UnitTypes eUnitType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT(szUnitClass));
...
{
	CvPlayer& kSellingPlayer = CvPlayerAI::getPlayer(eSellingPlayer);
	...
	if (getGold() >= iPriceToPay)
	{
		...
		//get City
		int iLoop;
		CvCity* locationToAppear = nullptr;
		...
		if (locationToAppear == nullptr && eLocationFlags.deepCoastal)
		{
			for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
			{
		...
		if (locationToAppear == nullptr && eLocationFlags.coastal)
		{
			for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
			{
				if (pLoopCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
		...
		if (locationToAppear == nullptr && eLocationFlags.inland)
		{
			for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
			{
				if (!pLoopCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
		...
		if (locationToAppear != nullptr || eLocationFlags.europe)
		{
			CvUnit* kBuyUnit = nullptr;
			...
			for (int iI = 0; iI < iNumUnits; iI++)
			{
				...
				{
					CvPlot* pStartingPlot = getStartingPlot();
					if (GC.getUnitInfo(eUnitType).getDomainType() == DOMAIN_SEA && pStartingPlot != nullptr)
					{
...
void CvPlayer::handleDiploEvent(DiploEventTypes eDiploEvent, PlayerTypes ePlayer, int iData1, int iData2)
{
	...
	case DIPLOEVENT_CONTACT:
		...
		CvPlayerAI::getPlayer(ePlayer).AI_setFirstContact(getID(), true);
		break;
	...
	case DIPLOEVENT_REJECTED_DEMAND:
		FAssertMsg(CvPlayerAI::getPlayer(ePlayer).getTeam() != getTeam(), "shouldn't call this function on our own team");

		AI_changeMemoryCount(ePlayer, MEMORY_REJECTED_DEMAND, 1);

		if (AI_demandRebukedSneak(ePlayer))
		{
			CvTeamAI::getTeam(getTeam()).AI_setWarPlan(CvPlayerAI::getPlayer(ePlayer).getTeam(), WARPLAN_PREPARING_LIMITED);
		}
		break;
	...
	case DIPLOEVENT_DEMAND_WAR:
		...
		CvTeamAI::getTeam(getTeam()).declareWar(CvPlayerAI::getPlayer(ePlayer).getTeam(), false, WARPLAN_LIMITED);
		break;
	...
	case DIPLOEVENT_JOIN_WAR:
		...
		CvTeamAI::getTeam(CvPlayerAI::getPlayer(ePlayer).getTeam()).declareWar(((TeamTypes)iData1), false, WARPLAN_DOGPILE);

		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == ((TeamTypes)iData1))
				{
					CvPlayerAI::getPlayer((PlayerTypes)iI).AI_changeMemoryCount(getID(), MEMORY_HIRED_WAR_ALLY, 1);
				}
			}
		}
		break;
	...
	case DIPLOEVENT_STOP_TRADING:
		...
		CvPlayerAI::getPlayer(ePlayer).stopTradingWithTeam((TeamTypes)iData1);
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == ((TeamTypes)iData1))
				{
					CvPlayerAI::getPlayer((PlayerTypes)iI).AI_changeMemoryCount(getID(), MEMORY_HIRED_TRADE_EMBARGO, 1);
				}
			}
		}
		break;
	...
	case DIPLOEVENT_ACCEPT_TAX_RATE:
		CvPlayerAI::getPlayer(ePlayer).changeTaxRate(iData1);
		break;
	...
	case DIPLOEVENT_ACCEPT_KING_GOLD:
		CvPlayerAI::getPlayer(ePlayer).changeGold(-iData1);
		CvPlayerAI::getPlayer(ePlayer).doREFReduction(iData1);
		/** NBMOD TAX **/
		CvPlayerAI::getPlayer(ePlayer).NBMOD_DecreaseMaxTaxRate();
	...
	case DIPLOEVENT_REFUSE_KING_GOLD:
		AI_changeMemoryCount(ePlayer, MEMORY_REFUSED_TAX, 1);
		/** NBMOD TAX **/
		CvPlayerAI::getPlayer(ePlayer).NBMOD_IncreaseMaxTaxRate();
	...
	case DIPLOEVENT_LIVE_AMONG_NATIVES:
		{
			CvUnit* pUnit = CvPlayerAI::getPlayer(ePlayer).getUnit(iData1);
			if (pUnit != nullptr)
			{
	...
	case DIPLOEVENT_ACQUIRE_MERC:
		{
			...
			CvPlayerAI::getPlayer(ePlayer).buyNativeMercs(getID(), pricetopay, mightbeangry);
		}
		break;
	...
	case DIPLOEVENT_ACQUIRE_NATIVE_SLAVE:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_NATIVE_SLAVE, 1, "TXT_KEY_BOUGHT_NATIVE_SLAVE", iData1);
		break;
	...
	case DIPLOEVENT_ACQUIRE_NOBLE:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_NOBLE, 1, "TXT_KEY_BOUGHT_NOBLE", iData1);
		break;
	// R&R, ray, Revolutionary Noble - END

	// R&R, ray, Bishop - START
	case DIPLOEVENT_ACQUIRE_BISHOP:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_BISHOP, 1, "TXT_KEY_BOUGHT_BISHOP", iData1);
		break;
	// R&R, ray, Bishop - END
	...
	case DIPLOEVENT_PAY_CHURCH_DEMAND:
		{
			int pricetopay = iData1;
			CvPlayer& kPlayer = CvPlayerAI::getPlayer(ePlayer); // human
						...
						case 1:
							{
								...
								CvPlayerAI::getPlayer(kPlayer.getParent()).AI_changeAttitudeExtra(ePlayer, 1); //attitude of King improved
					...
					case 1:
						{
							...
							CvPlayerAI::getPlayer(kPlayer.getParent()).AI_changeAttitudeExtra(ePlayer, -1);
			...
			if (kPlayer.getGold() >= pricetopay)
			{
				...
				switch (iFavourCase)
				{
					...
					case 1:
					{
						...
						gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
						break;
					}
					// change attitude of selected European
					case 2:
					{
						CvPlayer& kEuropePlayer = CvPlayerAI::getPlayer((PlayerTypes) iData2);
	...
	case DIPLOEVENT_CHURCH_WAR:
		{
			...
			CvPlayer& kPlayer = CvPlayerAI::getPlayer(ePlayer); // Human Player
			CvPlayer& kNativePlayer = CvPlayerAI::getPlayer(enemyID); // Native to declare war
			...
			if(choice == 1)
			{
				...
				CvPlayerAI::getPlayer(kPlayer.getParent()).AI_changeAttitudeExtra(ePlayer, 2); // Parent Attitude improvment
				...
				CvTeamAI::getTeam(kPlayer.getTeam()).declareWar(kNativePlayer.getTeam(),false, WARPLAN_LIMITED);
			...
			else if(choice == 2)
			{
				...
				if (locationToAppear != nullptr)
				{
					...
					UnitTypes ChurchReinforcementTypeArtillery = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(kPlayer.getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_KING_REINFORCEMENT_ARTIL"));
					...
					CvTeamAI::getTeam(kPlayer.getTeam()).declareWar(CvPlayerAI::getPlayer(enemyID).getTeam(),false, WARPLAN_LIMITED);
			...
			else
			{
				...
				CvPlayerAI::getPlayer(enemyID).AI_changeAttitudeExtra(ePlayer, 2);

				AI_changeAttitudeExtra(ePlayer, -2); // Church Attitude worsened
				CvPlayerAI::getPlayer(kPlayer.getParent()).AI_changeAttitudeExtra(ePlayer, -1); // Parent Attitude worsened
	...
	case DIPLOEVENT_COLONIAL_INTERVENTION_NATIVE_WAR:
		{
			...
			CvPlayer& kPlayer = CvPlayerAI::getPlayer(ePlayer); // Human Player
			CvPlayer& kNativePlayer = CvPlayerAI::getPlayer(NativeID); // Native to declare war
			...
			else
			{
				...
				{
					CvPlayer& kChurchPlayer = CvPlayerAI::getPlayer(eChurchPlayer);
				...
				{
					CvPlayer& kKingPlayer = CvPlayerAI::getPlayer(eChurchPlayer);
	...
	case DIPLOEVENT_COLONIES_AND_NATIVE_ALLIES_WAR_REFUSE:
		{
			...
			int IIDofRefusedEnemyNativePlayer = CvPlayerAI::getPlayer(ePlayer).getIDSecondPlayerFrenchNativeWar();
			//CvPlayer& HumanPlayer = CvPlayerAI::getPlayer(ePlayer);

			// increase Attitude of RefusedEnemyNativePlayer
			if (IIDofRefusedEnemyNativePlayer != NO_PLAYER)
			{
				CvPlayer& kRefusedEnemyNativePlayer = CvPlayerAI::getPlayer((PlayerTypes) IIDofRefusedEnemyNativePlayer);
			...
			{
				CvPlayer& kEnemyColonialPlayer = CvPlayerAI::getPlayer((PlayerTypes) iIDofEnemyColonialPlayer);
				CvWString szBuffer = gDLL->getText("TXT_KEY_COLONIES_AND_NATIVE_ALLIES_WAR_REFUSED", kEnemyColonialPlayer.getNameKey());
				gDLL->UI().addPlayerMessage(ePlayer, true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_POSITIVE_DINK", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
	...
	case DIPLOEVENT_COLONIES_AND_NATIVE_ALLIES_WAR_ACCEPT:
		{
			...
			int iIDofEnemyColonialPlayer = iData1;
			int IIDofDeclaredEnemyNativePlayer = CvPlayerAI::getPlayer(ePlayer).getIDSecondPlayerFrenchNativeWar();
			CvPlayer& HumanPlayer = CvPlayerAI::getPlayer(ePlayer);
			...
			if (pNativeAllyCapitolCity != nullptr)
			{
			...
			if (iIDofEnemyColonialPlayer != NO_PLAYER && IIDofDeclaredEnemyNativePlayer != NO_PLAYER)
			{
				...
				CvPlayer& ColonialEnemy = CvPlayerAI::getPlayer((PlayerTypes) iIDofEnemyColonialPlayer);
				CvPlayer& NativeEnemy = CvPlayerAI::getPlayer((PlayerTypes) IIDofDeclaredEnemyNativePlayer);

				// the allied Teams
				CvTeamAI& HumanTeam = CvTeamAI::getTeam(HumanPlayer.getTeam());
				CvTeamAI& NativeAllyTeam = CvTeamAI::getTeam(getTeam());

				// the enemies Teams
				CvTeamAI& ColonialEnemyTeam = CvTeamAI::getTeam(ColonialEnemy.getTeam());
				CvTeamAI& NativeEnemyTeam = CvTeamAI::getTeam(NativeEnemy.getTeam());
	...
	case DIPLOEVENT_ACQUIRE_SMUGGLERS:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_SMUGGLING_SHIP, 1, "TXT_KEY_BOUGHT_SMUGGLERS", iData1, LocationFlags::LocationFlagDeepCoastal, false);
		break;
	...
	case DIPLOEVENT_ACQUIRE_RANGERS:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_RANGER, 1, "TXT_KEY_TRAINED_RANGERS", iData1, LocationFlags::LocationFlagDeepCoastal);
		break;
	// R&R, ray, Rangers - END
	...
	case DIPLOEVENT_ACQUIRE_CONQUISTADORS:
		{
			...
			{
				CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_MOUNTED_CONQUISTADOR, 1, "TXT_KEY_HIRED_CONQUISTADORS", iData1, LocationFlags::LocationFlagDeepCoastal);
			}
			else
			{
				CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_CONQUISTADOR, 1, "TXT_KEY_HIRED_CONQUISTADORS", iData1, LocationFlags::LocationFlagDeepCoastal);
			}
		}
		break;
	...
	case DIPLOEVENT_ACQUIRE_PIRATES:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_PIRATE_FRIGATE, 1, "TXT_KEY_HIRED_PIRATES", iData1, LocationFlags::LocationFlagDeepCoastal, false);
		break;
	...
	case DIPLOEVENT_ACQUIRE_CONTINENTAL_GUARD:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_CONTINENTAL_GUARD, 1, "TXT_KEY_ACQUIRED_CONTINENTAL_GUARD", iData1, LocationFlags::LocationFlagDeepCoastal);
		break;
	// R&R, ray, Continental Guard - END

	// R&R, ray, Mortar - START
	case DIPLOEVENT_ACQUIRE_MORTAR:
		CvPlayerAI::getPlayer(ePlayer).buyUnitFromPlayer(getID(), UNITCLASS_MORTAR, 1, "TXT_KEY_ACQUIRED_MORTAR", iData1, LocationFlags::LocationFlagDeepCoastal);
		break;
	// R&R, ray, Mortar - END
	...
	case DIPLOEVENT_REV_SUPPORT:
	{
		...
		{
			CvWString szBuffer = gDLL->getText("TXT_KEY_REV_SUPPORT_ARRIVED", GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(getParent()).getLeaderType()).getDescription());
			CvPlayerAI::getPlayer(ePlayer).buyUnitFromParentPlayer(getID(), "UNITCLASS_REV_SUPPORT_LAND", GLOBAL_DEFINE_REV_SUPPORT_LAND, szBuffer, 0, LocationFlags::LocationFlagDeepCoastal, false, false);
		}
		else
		{
			CvWString szBuffer = gDLL->getText("TXT_KEY_REV_SUPPORT_ARRIVED", GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(getParent()).getLeaderType()).getDescription());
			CvPlayerAI::getPlayer(ePlayer).buyUnitFromParentPlayer(getID(), "UNITCLASS_REV_SUPPORT_SEA", GLOBAL_DEFINE_REV_SUPPORT_SEA, szBuffer, 0, LocationFlags::LocationFlagDeepCoastal, false, false);
		}
	}
		break;
	...
	case DIPLOEVENT_EUROPE_WAR:
		{
			...
			CvPlayer& kEnemyPlayer = CvPlayerAI::getPlayer(eEnemyPlayer);

			PlayerTypes eParent = kPlayer.getParent();
			PlayerTypes eEnemyParentPlayer = kEnemyPlayer.getParent();
			CvPlayer& kEnemyParentPlayer = CvPlayerAI::getPlayer(eEnemyParentPlayer);
			...
			if(choice == 1)
			{
				...
				//declaring limited war
				CvTeamAI::getTeam(kPlayer.getTeam()).declareWar(kEnemyPlayer.getTeam(),false, WARPLAN_LIMITED);
				...
				gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), NULL, NULL, false, false);
			}
	...
	case DIPLOEVENT_STEALING_IMMIGRANT:
		{
			...
			CvPlayer& victimPlayer = CvPlayerAI::getPlayer(victimID); // European we steal Immigrant
			CvPlayer& kPlayer = CvPlayerAI::getPlayer(ePlayer); // human Player
			...
			if (kPlayer.getGold() >= priceStealingImmigrant)
			{
				...
				if (NO_UNIT != eBestUnit)
				{
					...
					CvWString szBuffer;
					szBuffer = gDLL->getText("TXT_KEY_STEALING_IMMIGRANT_SUCCESSFULL", GC.getCivilizationInfo(CvPlayerAI::getPlayer(victimPlayer.getParent()).getCivilizationType()).getShortDescriptionKey(), GC.getCivilizationInfo(getCivilizationType()).getShortDescriptionKey(), GC.getUnitInfo(eBestUnit).getTextKeyWide());
					...
					if (randForVictimNoticing >= 1)
					{
						...
						CvWString szBuffer;
						szBuffer = gDLL->getText("TXT_KEY_STEALING_IMMIGRANT_NOTICED", GC.getCivilizationInfo(CvPlayerAI::getPlayer(victimPlayer.getParent()).getCivilizationType()).getShortDescriptionKey());
						gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
					}
				}
			}
		}
	...
	case DIPLOEVENT_EUROPE_PEACE:
		{
			...
			if(choice == 1)
			{
				...
				CvWString szBuffer = gDLL->getText("TXT_KEY_EUROPE_PEACE_ACCEPTED", GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(enemyID).getLeaderType()).getDescription());
			...
			else
			{
				...
				AI_changeAttitudeExtra(ePlayer, -1);
				CvWString szBuffer = gDLL->getText("TXT_KEY_EUROPE_PEACE_DENIED", GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(enemyID).getLeaderType()).getDescription());
				gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
		}
		break;
	...
	case DIPLOEVENT_ROYAL_INTERVENTION:
		{
			...
			{
				CvWString szBuffer = gDLL->getText("TXT_KEY_ROYAL_INTERVENTION_REFUSED", GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(enemyID).getLeaderType()).getDescription());
				gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DEAL_CANCELLED", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
		}
		break;
	...
	case DIPLOEVENT_PRIVATEERS_ACCUSATION:
		{
			...
			CvPlayer& kColonialParentPlayer = CvPlayerAI::getPlayer(parentID);
			...
			if (choice == 3)
			{
				CvTeamAI::getTeam(kPlayer.getTeam()).declareWar(CvPlayerAI::getPlayer(eColonialPlayer).getTeam(),false, WARPLAN_LIMITED);
			}
		}
		break;
	...
	case DIPLOEVENT_TARGET_CITY:
		pCity = CvPlayerAI::getPlayer((PlayerTypes)iData1).getCity(iData2);
		if (pCity != nullptr)
		{
...
bool CvPlayer::canTradeWith(PlayerTypes eWhoTo) const
{
	if (getParent() != eWhoTo)
	{
		CvPlayer& kWhoTo = CvPlayerAI::getPlayer(eWhoTo);
		...
		if (CvTeamAI::getTeam(getTeam()).isGoldTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isGoldTrading())
		{
			return true;
		}

		if (CvTeamAI::getTeam(getTeam()).isMapTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isMapTrading())
		{
			return true;
		}

		if (CvTeamAI::getTeam(getTeam()).isOpenBordersTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isOpenBordersTrading())
		{
			return true;
		}

		if (CvTeamAI::getTeam(getTeam()).isDefensivePactTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isDefensivePactTrading())
		{
			return true;
		}

		if (CvTeamAI::getTeam(getTeam()).isPermanentAllianceTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isPermanentAllianceTrading())
		{
			return true;
		}
	}
...
bool CvPlayer::canReceiveTradeCity(PlayerTypes eFromPlayer) const
{
	...
	if (isNative() && !CvPlayerAI::getPlayer(eFromPlayer).isNative())
	{
		return false;
	}
...
bool CvPlayer::canTradeItem(PlayerTypes eWhoTo, TradeData item, bool bTestDenial) const
{
	...
	case TRADE_PEACE:
		if (!(CvTeamAI::getTeam(getTeam()).isHuman()))
		{
			if (CvTeamAI::getTeam(getTeam()).isHasMet((TeamTypes)(item.m_iData1)) && CvTeamAI::getTeam(kWhoTo.getTeam()).isHasMet((TeamTypes)(item.m_iData1)))
			{
	...
	case TRADE_WAR:
		if (!(CvTeamAI::getTeam(getTeam()).isHuman()))
		{
			if (CvTeamAI::getTeam(getTeam()).isHasMet((TeamTypes)(item.m_iData1)) && CvTeamAI::getTeam(kWhoTo.getTeam()).isHasMet((TeamTypes)(item.m_iData1)))
			{
				if (CvTeamAI::getTeam(getTeam()).canDeclareWar((TeamTypes)(item.m_iData1)))
				{
					return true;
				}
			}
	...
	case TRADE_OPEN_BORDERS:
		if (getTeam() != kWhoTo.getTeam())
		{
			if (!atWar(getTeam(), kWhoTo.getTeam()))
			{
				if (!(CvTeamAI::getTeam(getTeam()).isOpenBorders(kWhoTo.getTeam())) && !isAlwaysOpenBorders() && !kWhoTo.isAlwaysOpenBorders())
				{
					if (CvTeamAI::getTeam(getTeam()).isOpenBordersTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isOpenBordersTrading())
					{
						return true;
	...
	case TRADE_DEFENSIVE_PACT:
		if (getParent() == NO_PLAYER || kWhoTo.getParent() == NO_PLAYER)
		{
			if (getTeam() != kWhoTo.getTeam())
			{
				if (!atWar(getTeam(), kWhoTo.getTeam()))
				{
					if (!(CvTeamAI::getTeam(getTeam()).isDefensivePact(kWhoTo.getTeam())))
					{
						if (CvTeamAI::getTeam(getTeam()).isDefensivePactTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isDefensivePactTrading())
						{
							if ((CvTeamAI::getTeam(getTeam()).getAtWarCount() == 0) && (CvTeamAI::getTeam(kWhoTo.getTeam()).getAtWarCount() == 0))
							{
								if (CvTeamAI::getTeam(getTeam()).canSignDefensivePact(kWhoTo.getTeam()))
								{
									return true;
								}
							}
	...
	case TRADE_PERMANENT_ALLIANCE:
		if (getTeam() != kWhoTo.getTeam())
		{
			if (!atWar(getTeam(), kWhoTo.getTeam()))
			{
				if (CvTeamAI::getTeam(getTeam()).isPermanentAllianceTrading() || CvTeamAI::getTeam(kWhoTo.getTeam()).isPermanentAllianceTrading())
				{
					if ((CvTeamAI::getTeam(getTeam()).getNumMembers() == 1) && (CvTeamAI::getTeam(kWhoTo.getTeam()).getNumMembers() == 1))
					{
						return true;
					}
				}
...
DenialTypes CvPlayer::getTradeDenial(PlayerTypes eWhoTo, TradeData item) const
{
	...
	case TRADE_MAPS:
		return CvTeamAI::getTeam(getTeam()).AI_mapTrade(kWhoTo.getTeam());
		break;

	case TRADE_PEACE:
		return CvTeamAI::getTeam(getTeam()).AI_makePeaceTrade(((TeamTypes)(item.m_iData1)), kWhoTo.getTeam());
		break;

	case TRADE_WAR:
		return CvTeamAI::getTeam(getTeam()).AI_declareWarTrade(((TeamTypes)(item.m_iData1)), kWhoTo.getTeam());
		break;
	...
	case TRADE_OPEN_BORDERS:
		return CvTeamAI::getTeam(getTeam()).AI_openBordersTrade(kWhoTo.getTeam());
		break;

	case TRADE_DEFENSIVE_PACT:
		return CvTeamAI::getTeam(getTeam()).AI_defensivePactTrade(kWhoTo.getTeam());
		break;

	case TRADE_PERMANENT_ALLIANCE:
		return CvTeamAI::getTeam(getTeam()).AI_permanentAllianceTrade(kWhoTo.getTeam());
		break;
...
bool CvPlayer::isTradingWithTeam(TeamTypes eTeam, bool bIncludeCancelable) const
{
	...
	for (CvDeal* pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != nullptr; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
	{
		if (bIncludeCancelable || pLoopDeal->isCancelable(getID()))
		{
			if (!pLoopDeal->isPeaceDeal())
			{
				if ((pLoopDeal->getFirstPlayer() == getID()) && (CvPlayerAI::getPlayer(pLoopDeal->getSecondPlayer()).getTeam() == eTeam))
				{
				...
				if ((pLoopDeal->getSecondPlayer() == getID()) && (CvPlayerAI::getPlayer(pLoopDeal->getFirstPlayer()).getTeam() == eTeam))
				{
...
void CvPlayer::stopTradingWithTeam(TeamTypes eTeam)
{
	...
	for (pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != nullptr; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
	{
		...
		{
			if (((pLoopDeal->getFirstPlayer() == getID()) && (CvPlayerAI::getPlayer(pLoopDeal->getSecondPlayer()).getTeam() == eTeam)) ||
				  ((pLoopDeal->getSecondPlayer() == getID()) && (CvPlayerAI::getPlayer(pLoopDeal->getFirstPlayer()).getTeam() == eTeam)))
			{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			...
			{
				CvPlayerAI::getPlayer((PlayerTypes)iI).AI_changeMemoryCount(getID(), MEMORY_STOPPED_TRADING, 1);
				CvPlayerAI::getPlayer((PlayerTypes)iI).AI_changeMemoryCount(getID(), MEMORY_STOPPED_TRADING_RECENT, 1);
			}
		}
	}
}
...
void CvPlayer::raze(CvCity* pCity)
{
	...
	{
		if (CvPlayerAI::getPlayer(eHighestCulturePlayer).getTeam() != getTeam())
		{
			CvPlayerAI::getPlayer(eHighestCulturePlayer).AI_changeMemoryCount(getID(), MEMORY_RAZED_CITY, 1);
			AI_changeMemoryCount(eHighestCulturePlayer, MEMORY_REVENGE_TAKEN, 1);
			if (isNative())
			{
				CvTeamAI::getTeam(getTeam()).AI_changeDamages(pCity->getTeam(), -AI_cityTradeVal(pCity, eHighestCulturePlayer));
			}
		}
	}
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (iI != getID())
			{
				if (pCity->isRevealed(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam(), false))
				{
	...
	if (pCity->getPreviousOwner() != NO_PLAYER)
	{
		...
		{
			UnitClassTypes eUnitClass = (UnitClassTypes) GC.getCivilizationInfo(CvPlayerAI::getPlayer(pCity->getPreviousOwner()).getCivilizationType()).getCapturedCityUnitClass();
			...
			if (eUnitClass != NO_UNITCLASS)
			{
				...
				if (eUnit != NO_UNIT)
				{
					for (int i = 0; i < pCity->getPopulation(); ++i)
					{
						...
						if (CvPlayerAI::getPlayer(pCity->getPreviousOwner()).isNative())
						{
		...
		iTreasure = iTreasure * GC.getCivilizationInfo(CvPlayerAI::getPlayer(pCity->getPreviousOwner()).getCivilizationType()).getTreasure() / 100;
		...
		for (int iTrait = 0; iTrait < GC.getNumTraitInfos(); ++iTrait)
		{
			...
			if (CvPlayerAI::getPlayer(pCity->getPreviousOwner()).isNative() && CvPlayerAI::getPlayer(pCity->getPreviousOwner()).hasTrait((TraitTypes) iTrait))
			{
				iTreasureMod += kTrait.getTreasureModifier();
			}
			// R&R, ray, for Trait Conquistador
			if (CvPlayerAI::getPlayer(pCity->getPreviousOwner()).isNative() && !isNative() && hasTrait((TraitTypes) iTrait))
			{
				iTreasureMod += kTrait.getTreasureModifier();
			}
		}
...
bool CvPlayer::canReceiveGoody(CvPlot* pPlot, GoodyTypes eGoody, const CvUnit* pUnit) const
{
	...
	if (kGoody.isWar())
	{
		...
		if (!CvTeamAI::getTeam(pPlot->getTeam()).canDeclareWar(getTeam()))
		{
			return false;
		}
	}
	...
	{
		if (pUnit == nullptr || !pUnit->canAcquirePromotionAny())
		{
			return false;
		}
	}
	...
	{
		if (pUnit == nullptr || pUnit->getDamage() < pUnit->maxHitPoints() * kGoody.getDamagePrereq() / 100)
		{
			return false;
		}
	}
	if (kGoody.isBad())
	{
		if (pUnit == nullptr || pUnit->isNoBadGoodies())
		{
			return false;
		}
	}
...
int CvPlayer::receiveGoody(CvPlot* pPlot, GoodyTypes eGoody, CvUnit* pUnit)
{
	...
	// }
	if (pUnit != nullptr && pUnit->getUnitInfo().getGoldFromGoodiesAndChiefsModifier() != 0)
	{
	...
	if (iGold != 0)
	{
		...
		if (pCity != nullptr)
		{
			for (int iTrait = 0; iTrait < GC.getNumTraitInfos(); ++iTrait)
			{
				...
				if (CvPlayerAI::getPlayer(pCity->getOwnerINLINE()).hasTrait(eTrait) || hasTrait(eTrait))
				{
	...
	if (iGoodyImmigrants > 0)
	{
		for (int gi = 0; gi < iGoodyImmigrants; gi++)
		{
			...
			if (NO_UNIT != eBestUnit)
			{
				...
				CvWString szBuffer;
				szBuffer = gDLL->getText("TXT_KEY_NEW_IMMIGRANT_AVAILABLE_SHORT", GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getShortDescriptionKey(),  GC.getUnitInfo(eBestUnit).getTextKeyWide());
	...
	if (!kGoody.isSpawnHostileUnitsAsXML() && !kGoody.isSpawnHostileAnimals() && !kGoody.isSpawnHostileNatives() && !kGoody.isSpawnHostileCriminals())
	{
		if (kGoody.getUnitClassType() != NO_UNITCLASS)
		{
			...
			if (eUnit != NO_UNIT)
			{
				...
				//treasure
				if (pGoodyUnit != nullptr && iGold != 0 && pGoodyUnit->getUnitInfo().isTreasure())
				{
	...
	if (kGoody.isWar())
	{
		CvTeamAI::getTeam(pPlot->getTeam()).declareWar(getTeam(), true, WARPLAN_EXTORTION);
	}
	...
	if (kGoody.isSpawnHostileUnitsAsXML() || kGoody.isSpawnHostileAnimals() || kGoody.isSpawnHostileNatives() || kGoody.isSpawnHostileCriminals())
	{
		...
		if (eHostileUnitClass != NO_UNITCLASS && barbarianPlayerTypes != NO_PLAYER)
		{
			...
			CvPlayer& barbarianPlayer = CvPlayerAI::getPlayer(barbarianPlayerTypes);
			...
			if (eUnit != NO_UNIT)
			{
				...
				for(int iI=0; iI<iActualHostileUnitsToSpawn; iI++)
				{
					...
					if (eHostileUnit != nullptr && iGold != 0 && eHostileUnit->getUnitInfo().isTreasure())
					{
...
bool CvPlayer::canFound(Coordinates foundCoord, bool bTestVisible) const
{
	...
	{
		if (!CvPlayerAI::getPlayer(pPlot->getOwnerINLINE()).isNative())
		{
			return false;
		}
	}
		...
		if (GC.getGameINLINE().isOption(GAMEOPTION_REDUCED_CITY_DISTANCE))
		{
			...
			for (iDX = -1; iDX <= 1 && bOwnsAll; iDX++)
			{
				for (iDY = -1; iDY <= 1 && bOwnsAll; iDY++)
				{
					...
					if (pLoopPlot != nullptr && pLoopPlot->getOwnerINLINE() != getID())
					{
						bOwnsAll = false;
					}
				}
			...
			for (iDY = -(iRange); iDY <= iRange; iDY++)
			{
				...
				if (pLoopPlot != nullptr)
				{
					if (pLoopPlot->isCity())
					{
						...
						{
							if (!CvPlayerAI::getPlayer(pLoopPlot->getOwnerINLINE()).isNative())
							{
								return false;
							}
						}
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	CvTeamAI& currentTeam = CvTeamAI::getTeam(getTeam());
...
void CvPlayer::processTrait(TraitTypes eTrait, int iChange)
{
	...
	if(getParent() != NO_PLAYER)
	{
		CvPlayerAI::getPlayer(getParent()).changeTotalPlayerAfricaSellProfitModifierInPercent(kTrait.getAfricaSellProfitModifierInPercent() * iChange);
		CvPlayerAI::getPlayer(getParent()).changeTotalPlayerPortRoyalSellProfitModifierInPercent(kTrait.getPortRoyalSellProfitModifierInPercent() * iChange);
	}
	// WTP, Africa and Port Royal Profit Modifiers - END
	...
	if (kTrait.getNativeAttitudeChange() != 0)
	{
		...
		{
			CvPlayer& kLoopPlayer = CvPlayerAI::getPlayer(eLoopPlayer);
...
void CvPlayer::processFatherOnce(FatherTypes eFather)
{
	...
	for (int iUnitClass = 0; iUnitClass < GC.getNumUnitClassInfos(); ++iUnitClass)
	{
		...
		if (eUnit != NO_UNIT)
		{
			...
			int iLoop;
			for (CvCity* pCity = firstCity(&iLoop); pCity != nullptr && pPlot == nullptr; pCity = nextCity(&iLoop))
			{
			...
			CvPlot* pPortPlot = nullptr;
			int iLoopWater;
			for (CvCity* pPortCity = firstCity(&iLoopWater); pPortCity != nullptr && pPortPlot == nullptr; pPortCity = nextCity(&iLoopWater))
			{
			...
			for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr && pPlot == nullptr; pLoopUnit = nextUnit(&iLoop))
			{
				CvPlot* pUnitPlot = pLoopUnit->plot();
				if (pUnitPlot != nullptr && pUnitPlot->isValidDomainForAction(eUnit))
				{
					pPlot = pUnitPlot;
				}
			}
			...
			{
				//WTP, ray fix for issue Free Water Units - START
				if (pPlot != nullptr && GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_LAND)
				// if (pPlot != nullptr)
				//WTP, ray fix for issue Free Water Units - END
				{
				...
				//WTP, ray fix for issue Free Water Units - START
				else if (pPortPlot != nullptr && GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_SEA)
				{
					OOS_LOG("Adding father unit", getTypeStr(eUnit));
				...
				{
					CvPlot* pStartingPlot = getStartingPlot();
					if (GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_SEA && pStartingPlot != nullptr)
					{
...
{
	CvTeamAI::getTeam(getTeam()).changeFatherPoints(ePointType, iChange * getFatherPointMultiplier() / 100);
}
...
void CvPlayer::changeGold(int iChange)
{
	...
	if (iChange > 0)
	{
		CvPlayerAI::getPlayer(getID()).AI_changeTotalIncome(iChange);
	}
}
...
{
	int iMultiplier = 100;
	if (!isHuman() && !CvPlayerAI::getPlayer(eOtherPlayer).isHuman())
	{
...
{
	return (m_iPopulationScore / std::max(1, CvTeamAI::getTeam(getTeam()).getNumMembers()));
}
...
{
	return (m_iLandScore  / std::max(1, CvTeamAI::getTeam(getTeam()).getNumMembers()));
}
...
void CvPlayer::setAlive(bool bNewValue)
{
	...
	else // coming alive or dying
	{
		...
		{
			CvTeamAI::getTeam(getTeam()).addAliveMember(getID());

			if (!isEverAlive())
			{
				m_bEverAlive = true;

				CvTeamAI::getTeam(getTeam()).addEverAliveMember(getID());
			}
			...
			if (GC.getGameINLINE().isMPOption(MPOPTION_SIMULTANEOUS_TURNS) || (GC.getGameINLINE().getNumGameTurnActive() == 0) || (GC.getGameINLINE().isSimultaneousTeamTurns() && CvTeamAI::getTeam(getTeam()).isTurnActive()))
			{
				setTurnActive(true);
			}
		...
		{
			CvTeamAI::getTeam(getTeam()).removeAliveMember(getID());
			...
			if (GC.getGameINLINE().getElapsedGameTurns() > 0)
			{
				...
				{
					if (CvPlayerAI::getPlayer(ePlayer).isAlive())
					{
						gDLL->UI().addPlayerMessage(ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_CIVDESTROYED", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
					}
				}
...
// {
// 	CvTeamAI& ownTeam = CvTeamAI::getTeam(getTeam());
//
...
void CvPlayer::buyEuropeSettlerIfLandlockedAI()
{
	...
	const CvPlayer& kParent = CvPlayerAI::getPlayer(getParent());
	...
	if (kParent.isAlive() && kParent.isEurope() && !::atWar(getTeam(), kParent.getTeam()) && (GC.getGameINLINE().getAIAutoPlay() == 0 || GC.getGameINLINE().getActivePlayer() != getID()))
	{
		...
		CvPlayerAI& kPlayer = CvPlayerAI::getPlayer(getID());
...
void CvPlayer::respawnTaxIncrease()
{
	...
	{
		CvDiploParameters* pDiplo = new CvDiploParameters(CvPlayerAI::getPlayer(getParent()).getID());
...
void CvPlayer::kill()
{
	...
	if (getCivCategoryTypes() == CIV_CATEGORY_EUROPEAN)
	{
		...
		for (PlayerTypes eLoopPlayer = FIRST_PLAYER; eLoopPlayer < NUM_PLAYER_TYPES; eLoopPlayer++)
		{
			if (CvPlayerAI::getPlayer(eLoopPlayer).isAlive())
			{
				gDLL->UI().addPlayerMessage((eLoopPlayer), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_CIVDESTROYED", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
			}
		}
	}
}
...
void CvPlayer::killMissionsAndTradeposts()
{
	for (PlayerTypes eLoopPlayer = FIRST_PLAYER; eLoopPlayer < NUM_PLAYER_TYPES; eLoopPlayer++)
	{
		...
		{
			int iLoop;
			for (CvCity* pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = kLoopPlayer.nextCity(&iLoop))
			{
		...
		else
		{
			...
			if (bDoTurn)
			{
				if (!GC.getGameINLINE().isMPOption(MPOPTION_SIMULTANEOUS_TURNS))
				{
					...
					{
						if (!CvTeamAI::getTeam(getTeam()).isTurnActive())
						{
							for (int iI = (getTeam() + 1); iI < MAX_TEAMS; iI++)
							{
								if (CvTeamAI::getTeam((TeamTypes)iI).isAlive())
								{
									CvTeamAI::getTeam((TeamTypes)iI).setTurnActive(true);
									break;
								}
							}
						}
					...
					else
					{
						for (int iI = (getID() + 1); iI < MAX_PLAYERS; iI++)
						{
							if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
							{
								if (GC.getGameINLINE().isPbem() && CvPlayerAI::getPlayer((PlayerTypes)iI).isHuman())
								{
								...
								{
									CvPlayerAI::getPlayer((PlayerTypes)iI).setTurnActive(true);
								}
								break;
							}
						}
...
void CvPlayer::setTeam(TeamTypes eTeam)
{
	...
	if (isAlive())
	{
		CvTeamAI::getTeam(getTeam()).changeAliveCount(-1);
	}
	if (isEverAlive())
	{
		CvTeamAI::getTeam(getTeam()).changeEverAliveCount(-1);
	}
	CvTeamAI::getTeam(getTeam()).changeNumCities(-(getNumCities()));
	CvTeamAI::getTeam(getTeam()).changeTotalLand(-(getTotalLand()));
...
bool CvPlayer::isYieldEuropeTradable(YieldTypes eYield) const
{
	...
	if (CvPlayerAI::getPlayer(getParent()).getYieldBuyPrice(eYield) <= 0)
	{
		return false;
	}
...
void CvPlayer::changeProfessionCombatChange(ProfessionTypes eIndex, int iChange)
{
	...
	if(iChange != 0)
	{
		...
		for (CvCity* pCity = firstCity(&iLoop); pCity != nullptr; pCity = nextCity(&iLoop))
		{
			pCity->AI_setWorkforceHack(true);
		}

		for (CvUnit* pUnit = firstUnit(&iLoop); pUnit != nullptr; pUnit = nextUnit(&iLoop))
		{
			...
			if (getCivic(eIndex) != NO_CIVIC)
			{
				if (getCivic(eIndex) != GC.getCivilizationInfo(getCivilizationType()).getCivilizationInitialCivics(eIndex))
				{
					for (int iI = 0; iI < MAX_PLAYERS; iI++)
					{
						if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
						{
							if (CvTeamAI::getTeam(getTeam()).isHasMet(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam()))
							{
...
void CvPlayer::updateGroupCycle(CvUnit* pUnit)
{
	...
	FAssertMsg(pUnit->getGroup() != nullptr, "Unit->getGroup() is not assigned a valid value");
	...
	iBestValue = MAX_INT;
	pBestSelectionGroupNode = nullptr;
	...
	if (pBestSelectionGroupNode != nullptr)
	{
...
{
	FAssert(pUnit != nullptr && pTransport != nullptr);
...
{
	CvDiploParameters* pDiplo = nullptr;
...
int CvPlayer::addTradeRoute(const IDInfo& kSource, const IDInfo& kDestination, YieldTypes eYield)
{
	...
	FAssert(pDestinationCity != nullptr || (kDestination.eOwner == getID() && kDestination.iID == CvTradeRoute::EUROPE_CITY_ID));
	FAssert(pDestinationCity == nullptr || pDestinationCity->getTeam() == getTeam());
...
bool CvPlayer::removeTradeRoute(int iId, bool bRemoveImport) // auto traderoute - Nightinggale
{
	...
	if (pDestinationCity != nullptr && bRemoveImport) // auto traderoute - Nightinggale
	{
...
CvTradeRoute* CvPlayer::getTradeRoute(int iId) const
{
	...
	FAssert(pTradeRoute == nullptr || pTradeRoute->getDestinationCity().eOwner == getID() || pTradeRoute->getSourceCity().eOwner == getID());
	return pTradeRoute;
}
...
{
	if (CvPlayerAI::getPlayer(eCityPlayer).getTeam() != getTeam())
	{
		return false;
	}
...
int CvPlayer::NBMOD_GetColonialMilitaryValue() const
{
    ...
    {
        iRevolutionWant = CvTeamAI::getTeam(getTeam()).getRebelPercent() + 50;
...
void CvPlayer::NBMOD_AddEuropeRandomUnit(bool bDisplay)
{
	...
	{
		CvPlayer& kParent = CvPlayerAI::getPlayer(getParent());
...
void CvPlayer::doBells()
{
	...
	if (!GC.getEraInfo(getCurrentEra()).isRevolution())
	{
		...
		else
		{
            ...
            {
                // DEBUG
                gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), CvWString::format(L"(EMW) %d vs. (KMW) %d",NBMOD_GetEuropeMilitaryValue(), NBMOD_GetColonialMilitaryValue()), nullptr, MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
                gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), CvWString::format(L"Start-MW: %d",NBMOD_REF_GetStartValue()), nullptr, MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
//            gDLL->UI().addPlayerMessage(getID() , true, GC.getEVENT_MESSAGE_TIME(), CvWString::format(L"Einheiten/Kapazit�t: %d/%d",NBMOD_GetNumEuropeUnits(),NBMOD_GetNumEuropeTransporting()), nullptr, MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
            }
	...
	case ADVANCEDSTARTACTION_CITY:
		{
			...
			if (bAdd)
			{
				if (0 == getNumCities())
				{
					...
					for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
					{
						...
						if (kPlayer.isAlive())
						{
							if (kPlayer.getTeam() == getTeam())
							{
								...
								{
									FAssert(kPlayer.getStartingPlot() != nullptr);
					...
					{
						CvPlot* pTempPlot = CvPlayerAI::getPlayer(eClosestPlayer).getStartingPlot();
						CvPlayerAI::getPlayer(eClosestPlayer).setStartingPlot(getStartingPlot(), false);
...
int CvPlayer::getAdvancedStartUnitCost(UnitTypes eUnit, bool bAdd, CvPlot* pPlot)
{
	...
	else
	{
		...
		if (0 == GC.getDefineINT("ADVANCED_START_ALLOW_UNITS_OUTSIDE_CITIES"))
		{
			...
			if (pCity == nullptr || pCity->getOwnerINLINE() != getID())
			{
				return -1;
			}
...
int CvPlayer::getAdvancedStartCityCost(bool bAdd, CvPlot* pPlot)
{
	...
	if (pPlot != nullptr)
	{
		...
		if (GC.getDefineINT("ADVANCED_START_CITY_PLACEMENT_MAX_RANGE") > 0)
		{
			...
			if (NO_PLAYER == eClosestPlayer || CvPlayerAI::getPlayer(eClosestPlayer).getTeam() != getTeam())
			{
				return -1;
			}
			//Only allow founding a city at someone elses start point if
			//We have no cities and they have no cities.
			if ((getID() != eClosestPlayer) && ((getNumCities() > 0) || (CvPlayerAI::getPlayer(eClosestPlayer).getNumCities() > 0)))
			{
				return -1;
			}
		}
					...
					if (pUnit != nullptr)
					{
						...
						if (pNearestCity != nullptr)
						{
							...
							if (pUnit->isBarbarian() && pUnit->getUnitInfo().isAnimal())
							{
								...
								gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, pLoopPlot, nullptr, MESSAGE_TYPE_INFO, pUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), true, true);
								iMaxCount--;
							}
							else
							{
		...
		if (eUnit != NO_UNIT)
		{
			...
			for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
			{
				...
				else if (iChange < 0)
				{
					...
					{
						int iUnitLoop;
						for (CvUnit* pUnit = firstUnit(&iUnitLoop); pUnit != nullptr; pUnit = nextUnit(&iLoop))
						{
...
void CvPlayer::createGreatGeneral(UnitTypes eGreatGeneralUnit, bool bIncrementExperience, const Coordinates coord)
{
	...
	if (bIncrementExperience)
	{
		...
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getTeam())
			{
				CvPlayerAI::getPlayer((PlayerTypes)iI).changeGreatGeneralsThresholdModifier(GC.getDefineINT("GREAT_GENERALS_THRESHOLD_INCREASE_TEAM"));
			}
		}
	}
	...
	if (pPlot)
	{
		...
		{
			CvWString szCity;
			szCity.Format(L"%s (%s)", pCity->getName().GetCString(), CvPlayerAI::getPlayer(pCity->getOwnerINLINE()).getName());
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (CvPlayerAI::getPlayer((PlayerTypes)iI).isAlive())
		{
			if (pPlot->isRevealed(CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam(), false))
			{
			...
			else
			{
				...
				gDLL->UI().addPlayerMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szMessage, "AS2D_UNIT_GREATPEOPLE", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_UNIT_TEXT"));
			}
		}
	}
}
...
void CvPlayer::createGreatAdmiral(UnitTypes eGreatAdmirallUnit, bool bIncrementExperience, const Coordinates coord)
{
	...
	if (bIncrementExperience)
	{
		...
		for (int iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (CvPlayerAI::getPlayer((PlayerTypes)iI).getTeam() == getTeam())
			{
				CvPlayerAI::getPlayer((PlayerTypes)iI).changeGreatAdmiralsThresholdModifier(GC.getDefineINT("GREAT_ADMIRALS_THRESHOLD_INCREASE_TEAM"));
			}
		}
	}
	...
	if (bOthers)
	{
		if (GC.getEventInfo(eEvent).isGlobal())
		{
			for (int i = 0; i < MAX_PLAYERS; i++)
			{
				if (i != getID())
				{
					CvPlayerAI::getPlayer((PlayerTypes)i).setEventOccured(eEvent, kEventTriggered, false);
				}
			}
		}
		else if (GC.getEventInfo(eEvent).isTeam())
		{
			for (int i = 0; i < MAX_PLAYERS; i++)
			{
				if (i != getID() && getTeam() == CvPlayerAI::getPlayer((PlayerTypes)i).getTeam())
				{
					CvPlayerAI::getPlayer((PlayerTypes)i).setEventOccured(eEvent, kEventTriggered, false);
				}
			}
		...
		if (bOthers)
		{
			if (kTrigger.isGlobal())
			{
				for (int i = 0; i < MAX_PLAYERS; i++)
				{
					if (i != getID())
					{
						CvPlayerAI::getPlayer((PlayerTypes)i).setTriggerFired(kTriggeredData, false, false);
					}
				}
			}
	...
	if (bAnnounce && !kTrigger.isTutorial())
	{
		...
		if (!kTriggeredData.m_szGlobalText.empty())
		{
			for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
			{
				...
				{
					if (CvTeamAI::getTeam(kLoopPlayer.getTeam()).isHasMet(getTeam()) && (NO_PLAYER == kTriggeredData.m_eOtherPlayer || CvTeamAI::getTeam(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()).isHasMet(getTeam())))
					{
						...
						if (bShowPlot)
						{
							if (kLoopPlayer.getTeam() != getTeam())
							{
								if (pPlot == nullptr || !pPlot->isRevealed(kLoopPlayer.getTeam(), false))
								{
									bShowPlot = false;
								}
							}
						...
						if (bShowPlot)
						{
							gDLL->UI().addPlayerMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szGlobalText, coord, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), true, true);
						}
						else
						{
		...
		{
			if (kTrigger.isShowPlot() && pPlot != nullptr && pPlot->isRevealed(getTeam(), false))
			{
				gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szText, coord, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), true, true);
			}
			else
			{
				gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
		}
	}
...
EventTriggeredData* CvPlayer::initTriggeredData(EventTriggerTypes eEventTrigger, bool bFire, int iCityId, int iPlotX, int iPlotY, PlayerTypes eOtherPlayer, int iOtherPlayerCityId, int iUnitId, BuildingTypes eBuilding)
{
	...
	CvCity* pOtherPlayerCity = nullptr;
	if (NO_PLAYER != eOtherPlayer)
	{
		pOtherPlayerCity = CvPlayerAI::getPlayer(eOtherPlayer).getCity(iOtherPlayerCityId);
	}
	...
	{
		if (pCity != nullptr && NO_BUILDING == eBuilding)
		{
			const InfoArray<BuildingClassTypes>& ReqBuildings = kTrigger.getBuildingsRequired();
	...
	// if (pUnit == nullptr && kTrigger.getNumUnits() > 0)
	if (pUnit == nullptr && kTrigger.getNumUnits() > 0 && kTrigger.isUnitsOnPlot())
	{
		return nullptr;
	}
	...
	if (pPlot == nullptr && pUnit != nullptr)
	{
		pPlot = pUnit->plot();
	}

	if (pPlot == nullptr && bPickPlot)
	{
		return nullptr;
	}
	...
	if (kTrigger.getNumUnitsGlobal() > 0)
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			{
				int iLoop;
				for (CvUnit* pLoopUnit = kLoopPlayer.firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = kLoopPlayer.nextUnit(&iLoop))
				{
	...
	if (kTrigger.isPickPlayer())
	{
		...
		if (NO_PLAYER == eOtherPlayer)
		{
			for (int i = 0; i < MAX_PLAYERS; i++)
			{
				if (CvPlayerAI::getPlayer((PlayerTypes)i).canTrigger(eEventTrigger, getID()))
				{
					if (kTrigger.isPickOtherPlayerCity())
					{
						...
						{
							pBestCity = CvPlayerAI::getPlayer((PlayerTypes)i).pickTriggerCity(eEventTrigger);
						}
	...
	if (pTriggerData != nullptr)
	{
		...
		pTriggerData->m_iCityId = pCity != nullptr ? pCity->getID() : -1;
		pTriggerData->m_iPlotX = pPlot != nullptr ? pPlot->getX_INLINE() : INVALID_PLOT_COORD;
		pTriggerData->m_iPlotY = pPlot != nullptr ? pPlot->getY_INLINE() : INVALID_PLOT_COORD;
		pTriggerData->m_eOtherPlayer = eOtherPlayer;
		pTriggerData->m_iOtherPlayerCityId = pOtherPlayerCity != nullptr ? pOtherPlayerCity->getID() : -1;
		pTriggerData->m_iUnitId = pUnit != nullptr ? pUnit->getID() : -1;
	...
	if (!isEmpty(kTrigger.getPythonCanDo()))
	{
		...
		else
		{
			...
			if (NO_PLAYER != eOtherPlayer)
			{
				pOtherPlayerCity = CvPlayerAI::getPlayer(eOtherPlayer).getCity(pTriggerData->m_iOtherPlayerCityId);
			}
	...
	if (aszTexts.size() > 0)
	{
		...
		pTriggerData->m_szText = gDLL->getText(aszTexts[iText].GetCString(),
			eOtherPlayer != NO_PLAYER ? CvPlayerAI::getPlayer(eOtherPlayer).getCivilizationAdjectiveKey() : L"",
			pCity != nullptr ? pCity->getNameKey() : L"",
			pUnit != nullptr ? pUnit->getNameOrProfessionKey() : L"",
			L"",
			NO_BUILDING != eBuilding ? GC.getBuildingInfo(eBuilding).getTextKeyWide() : L"",
			pOtherPlayerCity != nullptr ? pOtherPlayerCity->getNameKey() : L"",
			pPlot != nullptr && NO_TERRAIN != pPlot->getTerrainType() ? GC.getTerrainInfo(pPlot->getTerrainType()).getTextKeyWide() : L"",
			pPlot != nullptr && NO_IMPROVEMENT != pPlot->getImprovementType() ? GC.getImprovementInfo(pPlot->getImprovementType()).getTextKeyWide() : L"",
			pPlot != nullptr && NO_BONUS != pPlot->getBonusType() ? GC.getBonusInfo(pPlot->getBonusType()).getTextKeyWide() : L"",
			pPlot != nullptr && NO_ROUTE != pPlot->getRouteType() ? GC.getRouteInfo(pPlot->getRouteType()).getTextKeyWide() : L""
			);
...
bool CvPlayer::canDoEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	if (iGold != 0)
	{
		...
		{
			if (CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getGold() < iGold)
			{
				return false;
			}
		}
	...
	if (kEvent.isCityEffect())
	{
		...
		if (pCity == nullptr || !pCity->canApplyEvent(eEvent, kTriggeredData))
		{
			return false;
		}
	}
	else if (kEvent.isOtherPlayerCityEffect())
	{
		...
		CvCity* pCity = CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getCity(kTriggeredData.m_iOtherPlayerCityId);
		if (pCity == nullptr || !pCity->canApplyEvent(eEvent, kTriggeredData))
		{
			return false;
		}
	...
	{
		if (CvTeamAI::getTeam(getTeam()).isForceRevealedBonus((BonusTypes)kEvent.getBonusRevealed()))
		{
			return false;
		}
	}
	...
	if (0 != kEvent.getAttitudeModifier())
	{
		...
		if (CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam() == getTeam())
		{
			return false;
		}

		if (CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).isHuman())
		{
	...
	if (0 != kEvent.getTheirEnemyAttitudeModifier())
	{
		...
		TeamTypes eWorstEnemy = CvTeamAI::getTeam(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()).AI_getWorstEnemy();
		if (NO_TEAM == eWorstEnemy || eWorstEnemy == getTeam())
		{
			return false;
		}

		if (!CvTeamAI::getTeam(eWorstEnemy).isAlive())
		{
			return false;
		}
	...
	if (kEvent.isDeclareWar())
	{
		...
		if (!CvTeamAI::getTeam(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()).canDeclareWar(getTeam()) || !CvTeamAI::getTeam(getTeam()).canDeclareWar(CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getTeam()))
		{
			return false;
		}
	}
...
void CvPlayer::applyEvent(EventTypes eEvent, int iEventTriggeredId, bool bUpdateTrigger)
{
	...
	if (iGold != 0)
	{
		...
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer && kEvent.isGoldToPlayer())
		{
			...
			CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).changeGold(-iGold);
		}
	}
	...
	if (kEvent.isDeclareWar())
	{
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
		{
			CvTeamAI::getTeam(CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).getTeam()).declareWar(getTeam(), false, WARPLAN_LIMITED);
		}
	}
		...
		if (kEvent.getClearEventChance(iEvent) > 0)
		{
			...
			if (bClear)
			{
				if (kEvent.isGlobal())
				{
					...
					{
						CvPlayerAI::getPlayer((PlayerTypes)j).resetEventOccured((EventTypes)iEvent, j != getID());
					}
				}
				else if (kEvent.isTeam())
				{
					for (int j = 0; j < MAX_PLAYERS; j++)
					{
						if (getTeam() == CvPlayerAI::getPlayer((PlayerTypes)j).getTeam())
						{
							CvPlayerAI::getPlayer((PlayerTypes)j).resetEventOccured((EventTypes)iEvent, j != getID());
						}
					}
	...
	if (pCity != nullptr && kEvent.isCityEffect())
	{
		pCity->applyEvent(eEvent, *pTriggeredData, bClear);
	}
	else if (pOtherPlayerCity != nullptr && kEvent.isOtherPlayerCityEffect())
	{
		...
		if (kEvent.getMaxPillage() > 0)
		{
			...
			for (int i = 0; i < iNumPillage; ++i)
			{
				...
				for (int j = 0; j < GC.getMap().numPlotsINLINE(); ++j)
				{
					...
					if (pPlot != nullptr && pPlot->getOwnerINLINE() == getID() && pPlot->isCity())
					{
		...
		if (kEvent.getUnitClass() != NO_UNITCLASS)
		{
			...
			if (eUnit != NO_UNIT)
			{
				...
				if (GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_SEA)
				{
					...
					if (pUnitCity != nullptr && pUnitCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()) && pUnitCity->plot()->hasDeepWaterCoast())
					{
	...
	if (NO_BONUS != kEvent.getBonusRevealed())
	{
		CvTeamAI::getTeam(getTeam()).setForceRevealedBonus((BonusTypes)kEvent.getBonusRevealed(), true);
	}
	...
	if (0 != kEvent.getAttitudeModifier())
	{
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
		{
			...
			{
				CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).AI_changeMemoryCount(getID(), MEMORY_EVENT_GOOD_TO_US, kEvent.getAttitudeModifier());
			}
			else
			{
				CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).AI_changeMemoryCount(getID(), MEMORY_EVENT_BAD_TO_US, -kEvent.getAttitudeModifier());
			}
		}
	}
	...
	if (kEvent.getNumWorldNews() > 0)
	{
		...
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
		{
			eTheirWorstEnemy = CvTeamAI::getTeam(CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).getTeam()).AI_getWorstEnemy();
		}

		szGlobalText = gDLL->getText(kEvent.getWorldNews(iText).GetCString(),
			getCivilizationAdjectiveKey(),
			pCity != nullptr ? pCity->getNameKey() : L"",
			pTriggeredData->m_eOtherPlayer != NO_PLAYER ? CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).getCivilizationAdjectiveKey() : L"",
			pOtherPlayerCity != nullptr ? pOtherPlayerCity->getNameKey() : L"",
			L"",
			NO_TEAM != eTheirWorstEnemy ? CvTeamAI::getTeam(eTheirWorstEnemy).getName().GetCString() : L""
			);
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			if (kLoopPlayer.isAlive())
			{
				if (CvTeamAI::getTeam(kLoopPlayer.getTeam()).isHasMet(getTeam()) && (NO_PLAYER == pTriggeredData->m_eOtherPlayer || CvTeamAI::getTeam(CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).getTeam()).isHasMet(getTeam())))
				{
					...
					if (bShowPlot)
					{
						gDLL->UI().addPlayerMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), szGlobalText, coord, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), true, true);
					}
					else
					{
	...
	if (!isEmpty(kEvent.getLocalInfoTextKey()))
	{
		...
		szLocalText = gDLL->getText(kEvent.getLocalInfoTextKey(),
			...
			{
				gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szLocalText, coord, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), true, true);
			}
			else
			{
				gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szLocalText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
	...
	if (!isEmpty(kEvent.getOtherPlayerPopup()))
	{
		...
		if (NO_PLAYER != pTriggeredData->m_eOtherPlayer && CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).isHuman())
		{
			...
			if (pInfo != nullptr)
			{
				...
				CvPlayerAI::getPlayer(pTriggeredData->m_eOtherPlayer).addPopup(pInfo);
			}
		}
	}
...
void CvPlayer::expireEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData, bool bFail)
{
	...
	if (GC.getEventInfo(eEvent).isQuest())
	{
		...
		if (bFail)
		{
			gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), gDLL->getText(GC.getEventInfo(eEvent).getQuestFailTextKey()), "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
		}
	}
}
...
bool CvPlayer::checkExpireEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	CvPlayer& kPlayer = CvPlayerAI::getPlayer(kTriggeredData.m_ePlayer);
	...
	{
		if (kPlayer.getCity(kTriggeredData.m_iCityId) == nullptr)
		{
			return true;
		}
	}
	...
	{
		if (kPlayer.getUnit(kTriggeredData.m_iUnitId) == nullptr)
		{
			return true;
		}
	}
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		if (!CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).isAlive())
		{
			return true;
		}
		...
		{
			if (CvPlayerAI::getPlayer(kTriggeredData.m_eOtherPlayer).getCity(kTriggeredData.m_iOtherPlayerCityId) == nullptr)
			{
				return true;
			}
		}
...
bool CvPlayer::canTrigger(EventTriggerTypes eTrigger, PlayerTypes ePlayer) const
{
	...
	if (!CvTeamAI::getTeam(getTeam()).isHasMet(kPlayer.getTeam()))
	{
		return false;
	}
	...
	if (CvTeamAI::getTeam(getTeam()).isAtWar(kPlayer.getTeam()) != kTrigger.isOtherPlayerWar())
	{
		return false;
	}
...
CvCity* CvPlayer::pickTriggerCity(EventTriggerTypes eTrigger) const
{
	...
	if (pCity == nullptr && apCities.size() > 0)
	{
...
CvUnit* CvPlayer::pickTriggerUnit(EventTriggerTypes eTrigger, CvPlot* pPlot, bool bPickPlot) const
{
	...
	if (pUnit == nullptr && apUnits.size() > 0)
	{
...
int CvPlayer::getEventTriggerWeight(EventTriggerTypes eTrigger) const
{
	...
	if (ReqEvents.getLength() > 0)
	{
		...
		{
			if (getEventOccured(ReqEvents.getEvent(iI)) == nullptr)
			{
				bFoundValid = false;
				break;
			}
	...
	if (kTrigger.getMinOurLandmass() > 0 || kTrigger.getMaxOurLandmass() != -1)
	{
		...
		int iLoop;
		for (CvArea* pArea = GC.getMap().firstArea(&iLoop); pArea != nullptr; pArea = GC.getMap().nextArea(&iLoop))
		{
			if (!pArea->isWater())
			{
	...
	if (kTrigger.isProbabilityUnitMultiply() && kTrigger.getNumUnits() > 0)
	{
		...
		for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
		{
			if (MIN_INT != pLoopUnit->getTriggerValue(eTrigger, nullptr, true))
			{
				++iNumUnits;
			}
		}
...
{
	FAssert(CvTeamAI::getTeam(getTeam()).canChangeWarPeace(CvPlayerAI::getPlayer(ePlayer).getTeam()));

	CLinkList<TradeData> playerList;
	CLinkList<TradeData> loopPlayerList;
	TradeData kTradeData;
	setTradeItem(&kTradeData, TRADE_PEACE_TREATY, 0, nullptr);
...
CvCity* CvPlayer::getPopulationUnitCity(int iUnitId) const
{
	...
	for (CvCity* pCity = firstCity(&iLoop); pCity != nullptr; pCity = nextCity(&iLoop))
	{
		if (pCity->getPopulationUnitById(iUnitId) != nullptr)
		{
			return pCity;
		}
	}
	...
	if (iPrice != getYieldBuyPrice(eYield))
	{
		...
		if (bMessage)
		{
			...
			for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
			{
				CvPlayer& kLoopPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
				// R&R, ray price messages only displayed to Colony, not all players, as long as no according features
				// if (kLoopPlayer.isAlive() && CvTeamAI::getTeam(getTeam()).isHasMet(kLoopPlayer.getTeam()))
				if (kLoopPlayer.isAlive() && kLoopPlayer.getParent() == getID())
				{
					gDLL->UI().addPlayerMessage(kLoopPlayer.getID(), true, GC.getEVENT_MESSAGE_TIME(), szMessage, "AS2D_ADVISOR_SUGGEST", MESSAGE_TYPE_INFO, nullptr);
				}
			}
		}
	}
...
void CvPlayer::sellYieldUnitToEurope(CvUnit* pUnit, int iAmount, int iCommission)
{
	...
	if (pUnit != nullptr)
	{
		...
		FAssert(getParent() != NO_PLAYER);
		CvPlayer& kPlayerEurope = CvPlayerAI::getPlayer(getParent());
		...
		if (NO_YIELD != eYield)
		{
			...
			// R&R, ray, Smuggling - END
			{
				...
				{
					pUnit->setTransportUnit(nullptr);
	...
	CvPlayerAI::getPlayer(getParent()).doPrices();
...
// TAC - AI purchases military units - koma13 - END
{
	...
	// TAC - AI purchases military units - koma13 - START
	//iCost += CvTeamAI::getTeam(getTeam()).getEuropeUnitsPurchased((UnitClassTypes) kUnit.getUnitClassType()) * kUnit.getEuropeCostIncrease();
	if (bIncrease)
	{
		iCost += CvTeamAI::getTeam(getTeam()).getUnitsPurchasedHistory(kUnit.getUnitClassType()) * kUnit.getEuropeCostIncrease();
	}
	// TAC - AI purchases military units - koma13 - END
...
CvUnit* CvPlayer::buyYieldUnitFromAfrica(YieldTypes eYield, int iAmount, CvUnit* pTransport)
{
	...
	// PatchMod: Check Europe prices after each trade START
	CvPlayerAI::getPlayer(getParent()).doAfricaPrices();
	// PatchMod: Check Europe prices after each trade END
...
CvUnit* CvPlayer::buyYieldUnitFromPortRoyal(YieldTypes eYield, int iAmount, CvUnit* pTransport)
{
	...
	// PatchMod: Check Europe prices after each trade START
	CvPlayerAI::getPlayer(getParent()).doPortRoyalPrices();
	// PatchMod: Check Europe prices after each trade END
...
CvUnit* CvPlayer::buyEuropeUnit(UnitTypes eUnit, int iPriceModifier)
{
	...
	{
		changeGold(-iPrice);
		CvTeamAI::getTeam(getTeam()).changeUnitsPurchasedHistory(pUnit->getUnitClassType(), 1);
...
int CvPlayer::getAfricaUnitBuyPrice(UnitTypes eUnit) const
{
	...
	iCost += CvTeamAI::getTeam(getTeam()).getUnitsPurchasedHistory(kUnit.getUnitClassType()) * kUnit.getAfricaCostIncrease();
...
int CvPlayer::getPortRoyalUnitBuyPrice(UnitTypes eUnit) const
{
	...
	iCost += CvTeamAI::getTeam(getTeam()).getUnitsPurchasedHistory(kUnit.getUnitClassType()) * kUnit.getPortRoyalCostIncrease();
...
void CvPlayer::buyUnitsFromKing()
{
	...
	CvPlayer& kParent = CvPlayerAI::getPlayer(eParent);
...
YieldTypes CvPlayer::getHighestTradedYield() const
{
	...
	for (int i = 0; i < NUM_YIELD_TYPES; i++)
	{
		...
		if (isYieldEuropeTradable(eYield))
		{
			if (getCity(getHighestStoredYieldCityId(eYield)) != nullptr)
			{
...
void CvPlayer::changeTaxRate(int iChange)
{
	if (iChange != 0)
	{
		...
		if (eParent != NO_PLAYER)
		{
			...
			CvWString szBuffer = gDLL->getText(szTextKey.GetCString(), CvPlayerAI::getPlayer(eParent).getNameKey(), iOldRate, getTaxRate());
			gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_REVOLTSTART", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
		}
	}
}
...
void CvPlayer::doAction(PlayerActionTypes eAction, int iData1, int iData2, int iData3)
{
	...
	case PLAYER_ACTION_CONVINCE_FATHER:
		CvTeamAI::getTeam(getTeam()).convinceFather((FatherTypes) iData1, iData2 != 0);
		break;
...
bool CvPlayer::isInRevolution() const
{
	...
	if (!::atWar(getTeam(), CvPlayerAI::getPlayer(getParent()).getTeam()))
	{
		return false;
	}
...
bool CvPlayer::checkIndependence() const
{
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		CvPlayer& kParent = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);

		if (kParent.isAlive() && CvTeamAI::getTeam(kParent.getTeam()).isParentOf(getTeam()))
		{
			// land units on map
...
void CvPlayer::applyMissionaryPoints(CvCity* pCity)
{
	...
	{
		int iModifier = 100 + getMissionaryRateModifier() + CvPlayerAI::getPlayer(ePlayer).getMissionaryRateModifier();
		...
		if (getMissionaryPoints(ePlayer) >= iThreshold)
		{
			...
			if (eUnitClass != NO_UNITCLASS)
			{
				...
				if (eUnit != NO_UNIT)
				{
					CvUnit* pUnit = CvPlayerAI::getPlayer(ePlayer).initUnit(eUnit, GC.getUnitInfo(eUnit).getDefaultProfession(), pCity->coord());
					if (pUnit != nullptr)
					{
						bUnitCreated = true;
...
bool CvPlayer::canHaveMission(PlayerTypes ePlayer) const
{
	...
	if (CvPlayerAI::getPlayer(ePlayer).isNative())
	{
		return false;
	}

	if (::atWar(CvPlayerAI::getPlayer(ePlayer).getTeam(), getTeam()))
	{
		return false;
	}
	...
	if (!GC.getEraInfo(getCurrentEra()).isRevolution())
	{
		...
		if (eParent != NO_PLAYER)
		{
			if (getHighestTradedYield() != NO_YIELD)
			{
				...
				//iMultiplier += CvPlayerAI::getPlayer(eParent).AI_getAttitudeVal(getID()) * GC.getDefineINT("TAX_TRADE_THRESHOLD_ATTITUDE_PERCENT");
				...
				if (iTotalScore * 10000 > GC.getTAX_TRADE_THRESHOLD() * std::max(100, iMultiplier) * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getGrowthPercent())
				{
					...
					// R&R, ray, Improvements to Tax Mechanism - START
					int iTaxIncreaseChanceModifierFromKingAttitude = CvPlayerAI::getPlayer(eParent).AI_getAttitudeVal(getID()) * GC.getTAX_TRADE_INCREASE_CHANCE_KING_ATTITUDE_BASE();
	...
	if (getParent() != NO_PLAYER && canTradeWithEurope())
	{
		...
		if (NO_UNIT != eBestUnit)
		{
			...
			if(shortmessage) {
				...
				szBuffer = gDLL->getText("TXT_KEY_NEW_IMMIGRANT_AVAILABLE", GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getShortDescriptionKey(),  GC.getUnitInfo(eBestUnit).getTextKeyWide());
			}
...
void CvPlayer::buyLand(CvPlot* pPlot, bool bFree)
{
	...
	if (eOldOwner != NO_PLAYER)
	{
		...
		if (!CvTeamAI::getTeam(pPlot->getTeam()).isAtWar(getTeam()))
		{
			OOS_LOG("buy land war", pPlot->getTeam());
			CvPlayerAI::getPlayer(eOldOwner).changeGold((iGoldCost * GC.getDefineINT("BUY_PLOT_SELLER_INCOME_PERCENT")) / 100);
			CvPlayerAI::getPlayer(getID()).AI_changeGoldTradedTo(eOldOwner, iGoldCost);
		}
	}
...
void CvPlayer::buildTradeTable(PlayerTypes eOtherPlayer, CLinkList<TradeData>& ourList, const IDInfo& kTransport) const
{
	...
	if (::atWar(getTeam(), CvPlayerAI::getPlayer(eOtherPlayer).getTeam()))
	{
	...
	for (int i = NUM_BASIC_ITEMS; i < NUM_TRADEABLE_HEADINGS; i++)
	{
		...
		case TRADE_PEACE:
			if (!isHuman())
			{
				...
				{
					if (CvTeamAI::getTeam((TeamTypes)j).isAlive())
					{
						if (j != getTeam() && j != CvPlayerAI::getPlayer(eOtherPlayer).getTeam())
						{
...
bool CvPlayer::getItemTradeString(PlayerTypes eOtherPlayer, bool bOffer, bool bShowingCurrent, const TradeData& zTradeData, const IDInfo& kTransport, CvWString& szString, CvString& szIcon) const
{
	...
	case TRADE_YIELD:
		{
			...
			if (pTransport != nullptr)
			{
				int iAmount = bOffer ? CvPlayerAI::getPlayer(eOtherPlayer).getTradeYieldAmount(eYield, pTransport) : getTradeYieldAmount(eYield, pTransport);
	...
	case TRADE_CITIES:
		{
			CvCity* pCity = bOffer ? CvPlayerAI::getPlayer(eOtherPlayer).getCity(zTradeData.m_iData1) : getCity(zTradeData.m_iData1);
			if (pCity != nullptr)
			{
	...
	case TRADE_PEACE:
		if (bOffer)
		{
			...
			szString += CvTeamAI::getTeam((TeamTypes)zTradeData.m_iData1).getName();
		}
		else
		{
			szString = CvTeamAI::getTeam((TeamTypes)zTradeData.m_iData1).getName();
	...
	case TRADE_EMBARGO:
		...
		{
			szString = gDLL->getText("TXT_KEY_TRADE_STOP_TRADING_WITH");
			szString += L" " + CvTeamAI::getTeam((TeamTypes)zTradeData.m_iData1).getName();
		}
		else
		{
			szString = CvTeamAI::getTeam((TeamTypes)zTradeData.m_iData1).getName();
...
{
	for (CLLNode<TradeData>* pNode = ourInventory.head(); pNode != nullptr; pNode = ourInventory.next(pNode))
	{
		...
		{
			for (CLLNode<TradeData>* pOfferNode = theirOffer.head(); pOfferNode != nullptr; pOfferNode = theirOffer.next(pOfferNode))
			{
	...
	if (!isHuman() || !CvPlayerAI::getPlayer(eOtherPlayer).isHuman())  // everything allowed in human-human trades
	{
		CLLNode<TradeData>* pFirstOffer = ourOffer.head();
		if (pFirstOffer == nullptr)
		{
		...
		if (pFirstOffer != nullptr)
		{
			if (!CvDeal::isEndWar(pFirstOffer->m_data.m_eItemType) || !::atWar(getTeam(), CvPlayerAI::getPlayer(eOtherPlayer).getTeam()))
			{
				for (CLLNode<TradeData>* pNode = ourInventory.head(); pNode != nullptr; pNode = ourInventory.next(pNode))
				{
...
int CvPlayer::getMaxGoldTrade(PlayerTypes eOtherPlayer, const IDInfo& kTransport) const
{
	...
	if (pTradeCity != nullptr && pTradeCity->getOwnerINLINE() == getID())
	{
...
bool CvPlayer::checkPower(bool bReset)
{
	...
	for (CvUnit* pUnit = firstUnit(&iLoop); pUnit != nullptr; pUnit = nextUnit(&iLoop))
	{
		...
		CvArea* pArea = pPlot ? pPlot->area() : nullptr;
	...
	if (!isAchieveGained(eAchieve))
	{
		...
		if (bAnnounce)
		{
			if (isHuman())
			{
				...
				if (pPlot != nullptr)
				{
					gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, pPlot, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
				} else {
					gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, nullptr, MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
				}
				if (!GC.getGameINLINE().isGameMultiPlayer())
				{
...
{
	const CvPlayerAI& kPlayer = CvPlayerAI::getPlayer(getID());
...
//	gDLL->UI().addPlayerMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), nullptr, nullptr, true, true);
			...
			if (!afterMove || (afterMove && GC.getAchieveInfo((AchieveTypes)iI).isAfterMove()))
			{
				if (GC.getAchieveInfo((AchieveTypes)iI).getLowTurnNumber() <= GC.getGameINLINE().getGameTurn() && GC.getAchieveInfo((AchieveTypes)iI).getHighTurnNumber() >= GC.getGameINLINE().getGameTurn())
				{
					...
					if (GC.getAchieveInfo((AchieveTypes)iI).getNumEuroContacts() > 0)
					{
						...
						{
							CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes)iK);
							if(kPlayer.getCivilizationType() != NO_CIVILIZATION)
							{
								if (!kPlayer.isEurope() && kPlayer.getParent() != NO_PLAYER && kPlayer.getTeam() != getTeam() && CvTeamAI::getTeam(getTeam()).isHasMet(kPlayer.getTeam()))
								{
                                    count++;
								}
							}
					...
					if (GC.getAchieveInfo((AchieveTypes)iI).getNumNativeContacts() > 0)
					{
						...
						for (iK = 0; iK < MAX_PLAYERS; iK++)
						{
							...
							if(kPlayer.getCivilizationType() != NO_CIVILIZATION)
							{
								if (kPlayer.isNative()&& kPlayer.getTeam() != getTeam() && CvTeamAI::getTeam(getTeam()).isHasMet(kPlayer.getTeam()))
								{
                                    count++;
								}
							}
					...
					for (iK = 0; iK < GC.getNumBuildingInfos(); iK++)
					{
						...
						{
							count = 0;
							for (pCity = firstCity(&iJ); pCity != nullptr; pCity = nextCity(&iJ))
							{
...
//								if (pCity->isHasBuilding((BuildingTypes)iK))
					...
					for (iK = 0; iK < GC.getNumUnitInfos(); iK++)
					{
						...
						{
							count = 0;
							for (pUnit = firstUnit(&iJ); pUnit != nullptr; pUnit = nextUnit(&iJ))
							{
...
void CvPlayer::doAIImmigrant(int iIndex)
{
	if (getParent() != NO_PLAYER && canTradeWithEurope())
	{
		...
		if (NO_UNIT != eBestUnit)
		{
			m_aDocksNextUnits[iIndex] = pickBestImmigrant();
			CvUnit* pUnit = initEuropeUnit(eBestUnit);
			CvWString szBuffer = gDLL->getText("TXT_KEY_NEW_IMMIGRANT_AVAILABLE", GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getShortDescriptionKey(),  GC.getUnitInfo(eBestUnit).getTextKeyWide());
			gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_UNIT_GREATPEOPLE", MESSAGE_TYPE_INFO, GC.getUnitInfo(eBestUnit).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_UNIT_TEXT"));
			gDLL->getInterfaceIFace()->setDirty(EuropeScreen_DIRTY_BIT, true);
...
void CvPlayer::redistributeWood()
{
	...
	if (iTotalPopulation != 0)
	{
		...
		int iLoop2;
		for (pLoopCity = firstCity(&iLoop2); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop2))
		{
...
void CvPlayer::checkForNativeMercs()
{
	...
	for (int iPlayer=0;iPlayer<MAX_PLAYERS;iPlayer++)
	{
		CvPlayer& potentialMercPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		...
		if(potentialMercPlayer.isNative() && canContact((PlayerTypes) iPlayer) && potentialMercPlayer.AI_isWillingToTalk(getID()) && potentialMercPlayer.AI_getAttitude(getID(), false) >= GC.getLeaderHeadInfo(potentialMercPlayer.getPersonalityType()).getMinAttitudeGiveMercs() && !CvTeamAI::getTeam(getTeam()).isAtWar(potentialMercPlayer.getTeam()))
		{
...
void CvPlayer::checkForAfricanSlaves()
{
	...
	CvPlayer& King = CvPlayerAI::getPlayer(getParent());
	...
	if (!isHuman())
	{
		if (getGold() > pricetopay * 2)
		{
			...
			if (!AI().AI_isStrategy(STRATEGY_MILITARY_BUILDUP))
			{
				...
				CvPlayerAI::getPlayer(getParent()).changeGold(pricetopay);
				CvPlayerAI::getPlayer(getParent()).AI_changeAttitudeExtra(getID(), 5);
				changeGold(-pricetopay);
...
void CvPlayer::checkForBishop()
{
	...
	CvPlayer& Church = CvPlayerAI::getPlayer(eChurchPlayer);
...
void CvPlayer::buyNativeMercs(PlayerTypes sellingPlayer, int price, bool mightbeangry)
{
	...
	if(mightbeangry)
	{
		...
		{
			CvPlayerAI::getPlayer(sellingPlayer).AI_changeAttitudeExtra(getID(), -1);
		}
	}
}
//End TAC Native Mercs
...
void CvPlayer::checkForRevolutionSupport()
{
	...
	for (int iPlayer=0;iPlayer<MAX_PLAYERS;iPlayer++)
	{
		CvPlayer& potentialRevSupporter = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		...
		if(!potentialRevSupporter.isNative() && !potentialRevSupporter.isEurope() && potentialRevSupporter.getID() != getID() && !potentialRevSupporter.isInRevolution() && canContact((PlayerTypes) iPlayer) && potentialRevSupporter.AI_isWillingToTalk(getID()) && potentialRevSupporter.AI_getAttitude(getID(), false) >= GC.getDefineINT("MIN_ATTITUDE_FOR_SUPPORT") && !CvTeamAI::getTeam(getTeam()).isAtWar(potentialRevSupporter.getTeam()))
		{
			// Erik: We also require that the player is on good terms with the potential supporter's king
			if (potentialRevSupporter.getParent() != NO_PLAYER && CvPlayerAI::getPlayer(potentialRevSupporter.getParent()).AI_getAttitude(getID(), false) >= GC.getDefineINT("MIN_ATTITUDE_FOR_SUPPORT"))
			{
				//checking random chance for merc
				...
				if (supportChance > randomSupportValue)
				{
					...
					else
					{
						...
						pDiplo->addDiploCommentVariable(GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(potentialRevSupporter.getParent()).getLeaderType()).getDescription());
...
void CvPlayer::checkForEuropeanWars()
{
	...
	CvPlayer& potentialEnemy = CvPlayerAI::getPlayer((PlayerTypes) randomEnemyID);
	...
	if(!isHuman() && CvTeamAI::getTeam(getTeam()).isAtWar(potentialEnemy.getTeam()))
	{
		...
		{
			KingReinforcementType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_KING_REINFORCEMENT_SEA"));
		}
		else if (randomSupportChoice > 1)
		{
			KingReinforcementType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_KING_REINFORCEMENT_ARTIL"));
		}
		else
		{
			KingReinforcementType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_KING_REINFORCEMENT_LAND"));
		}
	...
	else if(isHuman() && !CvTeamAI::getTeam(getTeam()).isAtWar(potentialEnemy.getTeam()))
	{
		m_iTimerEuropeanWars = GC.getTIMER_EUROPEAN_WARS()*gamespeedMod/100;
		// handle this by DiploEvent with own king
		CvDiploParameters* pDiplo = new CvDiploParameters(getParent());
		...
		pDiplo->addDiploCommentVariable(GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(potentialEnemy.getParent()).getLeaderType()).getDescription());
...
void CvPlayer::checkForStealingImmigrant()
{
	...
	else
	{
		...
		CvPlayer& potentialVictim = CvPlayerAI::getPlayer((PlayerTypes) randomVictimID);

		// if potentialEnemy fulfills one of theses conditions do nothing
		if(getTeam() == potentialVictim.getTeam() || potentialVictim.isNative() || potentialVictim.isEurope() || potentialVictim.getID() == getID() || potentialVictim.isInRevolution() || !canContact((PlayerTypes) randomVictimID) || potentialVictim.getNumCities() < 1 || potentialVictim.getParent() == getParent() || CvTeamAI::getTeam(getTeam()).isAtWar(potentialVictim.getTeam()))
		{
			return;
		}
		...
		pDiplo->addDiploCommentVariable(GC.getCivilizationInfo(CvPlayerAI::getPlayer(potentialVictim.getParent()).getCivilizationType()).getShortDescriptionKey()); //getting Parent of Enemy for text
...
void CvPlayer::checkForConquistadors()
{
	...
	{
		CvPlayer& potentialNativeEnemy = CvPlayerAI::getPlayer((PlayerTypes)iJ);
...
void CvPlayer::checkForEuropeanPeace()
{
	...
	{
		CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		if (kPlayer.isAlive() && kPlayer.isPlayable() && CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(getTeam()) && !kPlayer.isInRevolution() && kPlayer.AI_isWillingToTalk(getID()))
		{
	...
	else
	{
		...
		//getting Parent of Enemy for Text
		CvPlayer& potentialEnemyToMakePeace = CvPlayerAI::getPlayer((PlayerTypes) eColonialAtWarWith);
		pDiplo->addDiploCommentVariable(GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(potentialEnemyToMakePeace.getParent()).getLeaderType()).getDescription());
		//setting ID of Enemy as Data
...
void CvPlayer::checkForRoyalIntervention()
{
	...
	{
		CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		if (kPlayer.isAlive() && kPlayer.isPlayable() && CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(getTeam()))
		{
			eColonialAtWarWith = (PlayerTypes) iPlayer;
			break;
		}
	...
	iGoldToPayBaseValue = iGoldToPayBaseValue * GC.getGameSpeedInfo(GC.getGame().getGameSpeedType()).getStoragePercent() / 100;
	int iGoldModiferByAttitude = GC.getDefineINT("ROYAL_INTERVENTIONS_PERCENT_GOLD_PRICE_CHANGE_PER_ATTITUDE_POINT");
	int iKingsAttitudeValue = CvPlayerAI::getPlayer(getParent()).AI_getAttitudeVal((PlayerTypes) getID(), false);
	int iGoldModifiedByAttitude = iGoldToPayBaseValue * (100 - iGoldModiferByAttitude * iKingsAttitudeValue) / 100;
	...
	if (isHuman())
	{
		...
		if (getGold() > iGoldModifiedByAttitude)
		{
			...
			pDiplo->addDiploCommentVariable(GC.getLeaderHeadInfo(CvPlayerAI::getPlayer(eColonialAtWarWith).getLeaderType()).getDescription());
	...
	for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
	{
		...
		if (pLoopUnit->getUnitInfo().isHiddenNationality() && pLoopUnit->getDomainType() == DOMAIN_SEA)
		{
			...
			if (pPlot != nullptr)
			{
				...
				if(ePossiblePlayer != NO_PLAYER && CvPlayerAI::getPlayer(ePossiblePlayer).getCivCategoryTypes() == CIV_CATEGORY_EUROPEAN)
				{
					...
					TeamTypes eOtherColonialPlayerTeam = CvPlayerAI::getPlayer(ePossiblePlayer).getTeam();
					if (eOtherColonialPlayerTeam != getTeam() && !CvTeamAI::getTeam(getTeam()).isAtWar(eOtherColonialPlayerTeam))
					{
...
bool CvPlayer::tryGetNewBargainPriceSell()
{
	...
	int attitudeLevel = CvPlayerAI::getPlayer(bargainPartner).AI_getAttitude(getID(), false);
	...
	if (randomValue < chanceToFail)
	{
		...
		CvPlayerAI::getPlayer(bargainPartner).setTimeNoTrade(randomAngry*gamespeedMod/100);
		CvPlayerAI::getPlayer(bargainPartner).AI_changeAttitudeExtra(getID(), -1);
		CvPlayerAI::getPlayer(bargainPartner).setWillingToBargain(false);
		return false;
	}
	...
	{
		CvPlayerAI::getPlayer(bargainPartner).setWillingToBargain(true);
		return true;
	}
...
bool CvPlayer::tryGetNewBargainPriceBuy()
{
	...
	if (randomValue < chanceToFail)
	{
		...
		CvPlayerAI::getPlayer(bargainPartner).setTimeNoTrade(randomAngry);
		CvPlayerAI::getPlayer(bargainPartner).AI_changeAttitudeExtra(getID(), -1);
		CvPlayerAI::getPlayer(bargainPartner).setWillingToBargain(false);
		return false;
	}
...
{
	return CvPlayerAI::getPlayer(ePlayer).AI_getAttitudeVal(getID());
}
// R&R, Robert Surcouf, No More Variables Hidden game option END
...
void CvPlayer::createEnemyPirates()
{
	...
	CvPlayer& barbarianPlayer = CvPlayerAI::getPlayer(GC.getGameINLINE().getBarbarianPlayer());
...
void CvPlayer::checkForMortar()
{
	...
	int iLoopPre;
	CvCity* pLoopCityPre = nullptr;
	for (pLoopCityPre = firstCity(&iLoopPre); pLoopCityPre != nullptr; pLoopCityPre = nextCity(&iLoopPre))
	{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		if (pLoopCity->getPopulation() >= minCitySize && !pLoopCity->isDisorder()) // check if city big enough and not in disorder
		{
			...
			for (int i = 0; i < pPlot->getNumUnits(); ++i)
			{
				...
				if (pLoopUnit != nullptr && pLoopUnit->getDomainType() == DOMAIN_LAND && pLoopUnit->canAttack() && pLoopUnit->getOwnerINLINE() == pLoopCity->getOwnerINLINE())
				{
...
void CvPlayer::checkForChurchContact()
{
	...
	CvPlayer& ChurchPlayer = CvPlayerAI::getPlayer(eChurchPlayer);
...
int CvPlayer::getUsedShipPrice(UnitClassTypes iUsedShipClassType) const
{
	...
	if (eUnit != NO_UNIT)
	{
		...
		int iKingsAttitude = CvPlayerAI::getPlayer(getParent()).AI_getAttitudeVal(getID());
...
UnitClassTypes CvPlayer::getRandomUsedShipClassTypeID() const
{
	...
	for (UnitClassTypes iI = FIRST_UNITCLASS; iI < NUM_UNITCLASS_TYPES; ++iI)
	{
		...
		if (iUnitClassCompareRand > iBestLastCompareValue)
		{
			...
			if (eUnit != nullptr && GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_SEA && GC.getUnitInfo(eUnit).getEuropeCost() > 0 && getGold() > GC.getUnitInfo(eUnit).getEuropeCost())
			{
				eBestUnitClass = iI;
...
void CvPlayer::decreaseCounterForUsedShipDeals()
{
	if (m_iTimerUsedShips > 0)
	{
		...
		if (isKingWillingToTradeUsedShips())
		{
			...
			gDLL->UI().addPlayerMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_POSITIVE_DINK", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
		}
	}
}
...
void CvPlayer::acquireUsedShip(UnitClassTypes iUsedShipClassType, int iPrice)
{
	...
	if (eUnit != NO_UNIT && pStartingPlot != nullptr)
	{
...
int CvPlayer::getForeignImmigrantPrice(UnitClassTypes iForeignImmigrantClassType, int iForeignKingID) const
{
	...
	if (eUnit != NO_UNIT)
	{
		...
		// we modify the price by the attitude of the King
		int iKingsAttitude = CvPlayerAI::getPlayer((PlayerTypes) iForeignKingID).AI_getAttitudeVal(getID());
		iDiscountRandPercent = iDiscountRandPercent + iKingsAttitude;
...
UnitClassTypes CvPlayer::getRandomForeignImmigrantClassTypeID(int iKingID) const
{
	...
	for (UnitClassTypes iI = FIRST_UNITCLASS; iI < NUM_UNITCLASS_TYPES; ++iI)
	{
		...
		if (iUnitClassCompareRand > iBestLastCompareValue)
		{
			...
			if (eUnit != nullptr && GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_LAND && GC.getUnitInfo(eUnit).getEuropeCost() > 0 && getGold() > GC.getUnitInfo(eUnit).getEuropeCost())
			{
...
bool CvPlayer::isForeignKingWillingToTradeImmigrants(int iForeignKingID)
{
	...
	CvPlayerAI& kForeignKing = CvPlayerAI::getPlayer((PlayerTypes) iForeignKingID);
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (kPlayer.isAlive() && ePlayer != getID() && kPlayer.isPlayable())
		{
			...
			if (kPlayer.getParent() == kForeignKing.getID())
			{
				...
				if (CvTeamAI::getTeam(getTeam()).isAtWar(kPlayer.getTeam()))
				{
...
void CvPlayer::checkForChurchWar()
{
	...
	CvPlayer& potentialNativeEnemy = CvPlayerAI::getPlayer((PlayerTypes) randomEnemyID);
	...
	for (pNativeLoopCity = potentialNativeEnemy.firstCity(&iNativeLoop); pNativeLoopCity != nullptr; pNativeLoopCity = potentialNativeEnemy.nextCity(&iNativeLoop))
	{
	...
	if(!isHuman() && CvTeamAI::getTeam(getTeam()).isAtWar(potentialNativeEnemy.getTeam()))
	{
		//Randomize which unit AI gets
		...
		if (randomSupportChoice > 1)
		{
			ChurchReinforcementType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_KING_REINFORCEMENT_ARTIL"));
		}
		else
		{
			ChurchReinforcementType = (UnitTypes)GC.getCivilizationInfo(CvPlayerAI::getPlayer(getParent()).getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_MOUNTED_CONQUISTADOR"));
		}
	...
	else if(isHuman() && !CvTeamAI::getTeam(getTeam()).isAtWar(potentialNativeEnemy.getTeam()))
	{
		// reset timer
		m_iTimerChurchWar = GC.getTIMER_CHURCH_WAR()*gamespeedMod/100;
...
void CvPlayer::checkForColonialInterventionInNativeWar()
{
	...
	{
		CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		if (kPlayer.isAlive() && kPlayer.isNative() && CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(getTeam()))
		{
	...
	{
		CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		CvPlayer& kNativePlayer = CvPlayerAI::getPlayer((PlayerTypes) eNativeAtWarWith);
		if (kPlayer.isAlive() && kPlayer.isPlayable() && !CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(getTeam()) && !CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(kNativePlayer.getTeam()))
		{
	...
	CvPlayer& kNativeAtWarWith = CvPlayerAI::getPlayer((PlayerTypes) eNativeAtWarWith);
...
void CvPlayer::checkForColonialAndNativeAlliesWar()
{
	...
	{
		CvPlayer& kPlayer = CvPlayerAI::getPlayer((PlayerTypes) iPlayer);
		if (kPlayer.isAlive() && !kPlayer.isNative() && !kPlayer.isEurope() && CvTeamAI::getTeam(kPlayer.getTeam()).isAtWar(getTeam()))
		{
	...
	for (PlayerTypes ePlayer = FIRST_PLAYER; ePlayer < NUM_PLAYER_TYPES; ++ePlayer)
	{
		CvPlayer& kFirstNativePlayer = CvPlayerAI::getPlayer(ePlayer);
		CvPlayer& kColonialPlayer = CvPlayerAI::getPlayer(eColonialPlayerAtWarWith);
		if (kFirstNativePlayer.isAlive() && kFirstNativePlayer.isNative() && !CvTeamAI::getTeam(kFirstNativePlayer.getTeam()).isAtWar(getTeam()) && !CvTeamAI::getTeam(kFirstNativePlayer.getTeam()).isAtWar(kColonialPlayer.getTeam()))
		{
	...
	for (PlayerTypes ePlayer = FIRST_PLAYER; ePlayer < NUM_PLAYER_TYPES; ++ePlayer)
	{
		...
		{
			CvPlayer& kSecondNativePlayer = CvPlayerAI::getPlayer(ePlayer);
			CvPlayer& kFirstNativePlayerToCompare = CvPlayerAI::getPlayer(eFirstNativeDiploPlayer);
			CvPlayer& kColonialPlayer = CvPlayerAI::getPlayer(eColonialPlayerAtWarWith);
			// here we also check that the 2 Native Players are not at war
			if (kSecondNativePlayer.isAlive() && kSecondNativePlayer.isNative() && !CvTeamAI::getTeam(kSecondNativePlayer.getTeam()).isAtWar(getTeam()) && !CvTeamAI::getTeam(kSecondNativePlayer.getTeam()).isAtWar(kColonialPlayer.getTeam()) && !CvTeamAI::getTeam(kSecondNativePlayer.getTeam()).isAtWar(kFirstNativePlayerToCompare.getTeam()))
			{
	...
	CvPlayer& kColonialAtWarWith = CvPlayerAI::getPlayer((PlayerTypes) eColonialPlayerAtWarWith);
	pDiplo->addDiploCommentVariable(kColonialAtWarWith.getCivilizationDescription());

	//getting Native Enemy Civ for text
	CvPlayer& kNativeAtWarWith = CvPlayerAI::getPlayer((PlayerTypes) eSecondNativeDiploPlayer);
	pDiplo->addDiploCommentVariable(kNativeAtWarWith.getCivilizationDescription());
...
bool CvPlayer::isYieldAfricaTradable(YieldTypes eYield) const
{
	...
	if (CvPlayerAI::getPlayer(getParent()).getYieldAfricaBuyPrice(eYield) <= 0)
	{
		return false;
	}
...
bool CvPlayer::isYieldPortRoyalTradable(YieldTypes eYield) const
{
	...
	if (CvPlayerAI::getPlayer(getParent()).getYieldPortRoyalBuyPrice(eYield) <= 0)
	{
		return false;
	}
...
namespace
{
	bool compareUnitValue(CvUnit* pUnitA, CvUnit* pUnitB)
	{
		...
		const int iUnitValueA = pUnitA->canFound(nullptr) ? INT_MAX : abs(pUnitA->getUnitInfo().getEuropeCost());
		const int iUnitValueB = pUnitB->canFound(nullptr) ? INT_MAX : abs(pUnitB->getUnitInfo().getEuropeCost());
...
void CvPlayer::writeDesyncLog(FILE *f) const
{
	...
	int iLoop;
	for (const CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
